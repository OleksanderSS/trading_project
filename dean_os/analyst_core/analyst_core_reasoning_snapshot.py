...
from dean_os.analyst_core.contracts import (
    ANALYST_REASONING_RECEIPT_CONTRACT,
    ANALYST_REASONING_SNAPSHOT_CONTRACT,
)
...
SNAPSHOT_CONTRACT = ANALYST_REASONING_SNAPSHOT_CONTRACT
REASONING_RECEIPT_CONTRACT = ANALYST_REASONING_RECEIPT_CONTRACT
...
class AnalystCoreReasoningSnapshot:
    ...
    def build(
        ...
        *,
        ...
        hypothesis_journal_path: str | Path | None = None,
        save: bool = True,
    ) -> dict[str, Any]:
        ...
        projection = (
            project_active_hypotheses(
                hypothesis_journal_path,
                domain_id=domain_id,
                as_of=as_of,
            )
            if hypothesis_journal_path is not None
            else {
                "active_hypotheses": [],
        ...
        packet = AnalysisPacket(
            ...
            hypotheses=list(projection["active_hypotheses"]),
        )
        ...
        proposal_rows = list(enriched.hypothesis_review_proposals)
        delta_rows = [item.model_dump(mode="json") for item in deltas]
        analysis_output_sha256 = _sha256_json(
            enriched.model_dump(mode="json")
        )
        delta_sha256s = [_sha256_json(item) for item in delta_rows]
        receipt_identity = {
            "domain_id": domain_id,
            "as_of": as_of,
            ...
            "hypothesis_ids": [
                str(item.get("hypothesis_id")) for item in hypothesis_rows
            ],
            "proposal_ids": [
                str(item.get("proposal_id")) for item in proposal_rows
            ],
            "hypothesis_journal_tip_sha256": projection.get(
                "journal_tip_sha256"
            ),
        }
        reasoning_receipt = {
            ...
            + _sha256_json(receipt_identity)[:24],
            **receipt_identity,
        ...
        payload = {
            ...
            "inputs": {
                ...
                "hypothesis_journal": {
                    "path": (
                        str(hypothesis_journal_path)
                        if hypothesis_journal_path is not None
                        else None
                    ),
                    "tip_sha256": projection.get("journal_tip_sha256"),
                    "record_count": projection.get("journal_record_count"),
                },
            },
            "summary": {
                "evidence_count": len(evidence),
                ...
                "prior_hypothesis_count": projection.get(
                    "active_hypothesis_count", 0
                ),
                "hypothesis_review_proposal_count": len(proposal_rows),
            ...
            "hypothesis_ledger": hypothesis_rows,
            "hypothesis_review_proposals": proposal_rows,
            "hypothesis_projection": {
                key: value
                for key, value in projection.items()
                if key != "active_hypotheses"
            },
            "evidence_gaps": [
                item.model_dump(mode="json") for item in enriched.evidence_gaps
            ],
            "reasoning_delta_trail": delta_rows,
