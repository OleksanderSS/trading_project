...
CONTRACT = "dean_pipeline_control_saved_data_coverage_v1"
...
class PipelineControlSavedDataCoverage:
    ...
    def build(
        ...
        *,
        ...
        configured_tickers: list[str] | None = None,
        ...
        if configured_tickers is None:
            ...
            if not configured_assets:
                raise ValueError("configured_tickers must not be empty")
            sector_by_asset = {ticker: [] for ticker in configured_assets}
        ...
        payload = {
            ...
            "inputs": {
                ...
                "configured_tickers_override": (
                    configured_assets if configured_tickers is not None else None
                ),
...
def _analyze_price_source(
    ...
    *,
    ...
    for ticker in configured_assets:
        ticker_group = frame.loc[frame["ticker"].eq(ticker)].copy()
        if "interval" in ticker_group.columns:
            intervals = ticker_group["interval"].dropna().unique()
            if len(intervals) > 0:
                for interval in intervals:
                    sub_group = ticker_group.loc[ticker_group["interval"].eq(interval)].copy()
                    contexts.append(
                        _analyze_context(
                            sub_group,
                            path=path,
                            ticker=ticker,
                            timeframe=str(interval).lower(),
                ...
                continue
        # Fallback to legacy timeframe if interval column is missing or empty
        contexts.append(
            _analyze_context(
                ticker_group,
                path=path,
...
__all__ = ["CONTRACT", "PipelineControlSavedDataCoverage"]
