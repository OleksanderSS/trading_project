...
from dean_os.domain_sector_market_coverage_bridge import (
    CONTRACT as DOMAIN_COVERAGE_BRIDGE_CONTRACT,
...
from dean_os.pipeline_control.pipeline_control_saved_data_coverage import (
    CONTRACT as GENERIC_COVERAGE_CONTRACT,
)
    ...
    def build(
        ...
        *,
        ...
        if domain_id is not None:
            verified_coverage = load_verified_domain_sector_market_coverage_bridge(
                coverage_path,
                expected_domain_id=domain_id,
            )
            contexts = list(verified_coverage.get("eligible_contexts") or [])
            required_tickers = sorted(
                set(
                    (verified_coverage.get("required_market_scope") or {}).get(
                        "required_tickers"
                    )
                    or []
                )
            )
            coverage_contract = DOMAIN_COVERAGE_BRIDGE_CONTRACT
        else:
            _verify_generic_coverage(coverage)
            contexts = [
                context
            ...
            required_tickers = sorted(
                {str(context.get("ticker") or "").upper() for context in contexts}
            )
            coverage_contract = GENERIC_COVERAGE_CONTRACT
        if not contexts:
            raise ValueError("Coverage contains no eligible 15m contexts.")
        context_tickers = [str(item.get("ticker") or "").upper() for item in contexts]
        if len(context_tickers) != len(set(context_tickers)):
            raise ValueError("Coverage contains duplicate eligible 15m ticker contexts.")
        if sorted(context_tickers) != required_tickers:
            raise ValueError("Coverage eligible 15m ticker scope is incomplete.")
        for context in contexts:
            start = pd.to_datetime(
                context.get("effective_start"), errors="coerce", utc=True
            )
            if pd.isna(start):
                raise ValueError("Coverage eligible context effective_start is required.")
        source_paths = {
            str(Path(str(context.get("source_path") or "")).resolve())
            for context in contexts
        ...
        for context in contexts:
            if context.get("source_sha256") != source_sha:
                raise ValueError("Coverage eligible context source hash mismatch.")
        ...
        payload = {
            ...
            "source_provenance": {
                ...
                "coverage_bridge_verified": domain_id is not None,
            },
...
def _build_clean_15m(
    ...
    for context in contexts:
        ...
        if pd.isna(start):
            raise ValueError(f"Eligible context for {ticker} has no effective_start.")
...
def _verify_generic_coverage(payload: dict[str, Any]) -> None:
    if payload.get("contract") != GENERIC_COVERAGE_CONTRACT:
        raise ValueError("Unsupported generic coverage contract.")
    if payload.get("mode") != "pipeline_control_saved_data_coverage":
        raise ValueError("Unsupported generic coverage mode.")
    if (payload.get("summary") or {}).get(
        "coverage_status"
    ) != "saved_data_coverage_ready":
        raise ValueError("Generic coverage is not ready.")
