...
def load_duckdb_tables(
    *,
    ...
    try:
        ...
        snapshot_path = db_path.with_name(f"{db_path.stem}_snapshot{db_path.suffix}")
        try:
            shutil.copy2(db_path, snapshot_path)
            logging.info(f"DuckDB locked. Created snapshot at {snapshot_path}")
        except PermissionError:
            logging.info(f"DuckDB locked and could not be copied. Using existing snapshot if available: {snapshot_path}")
