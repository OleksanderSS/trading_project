...
class EventCausalGraphBuilder:
    ...
    Usage:
        ...
        from dean_os.draft.dean_os_agent_system_v7.dean_os.event_causal_graph import EventCausalGraphBuilder
    ...
    def build(self, event: Any) -> CausalGraph:
        ...
        # Second-order: high-impact first-order nodes get downstream effects
            ...
            for sector in parent.affected_sectors[:2]:
                ...
                edges.append(CausalEdge(
                    ...
                    causal_metadata=CausalClaimMetadata(
                        ...
                        causal_claim_allowed=False,
                        limitations=[
                            "Second-order propagation is a review hypothesis"
                        ],
                    ),
                    dynamics=GraphEdgeDynamics(
                        strength=0.7,
                        ...
                        regime_dependencies=list(parent.affected_sectors),
                        evidence_count=0,
                        decay_function="unknown",
                        activation_state="candidate",
                    ),
        ...
        # Prioritise context_tickers in watch list
        watch_list = sorted(
            all_tickers,
            key=lambda t: (0 if t in self.context_tickers else 1, t),
        )

        overall_confidence = round(
            sum(n.estimate_confidence for n in nodes if n.depth > 0)
            / max(len(nodes) - 1, 1),
            3,
