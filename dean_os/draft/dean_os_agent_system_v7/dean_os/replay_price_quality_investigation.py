...
from dean_os.draft.dean_os_agent_system_v7.dean_os.market_data_api import prepare_market_frame, read_market_frame
...
def _load_reports(paths: list[str | Path]) -> list[dict[str, Any]]:
    ...
    for path in paths:
        try:
            ...
            reports.append({"path": str(path), "loaded": True, "payload": payload, "mode": payload.get("mode")})
        except Exception as exc:
            reports.append({"path": str(path), "loaded": False, "error": str(exc)})
    return reports
...
def _diagnose_artifact(
    ...
    try:
        ...
        raw = read_market_frame(pd, resolved)
        frame = prepare_market_frame(pd, raw, close_col=close_col, datetime_col=datetime_col)
    except Exception as exc:
