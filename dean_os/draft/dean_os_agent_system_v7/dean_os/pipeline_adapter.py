...
from dean_os.draft.dean_os_agent_system_v7.dean_os.context_evidence_provenance import audit_news_records
...
class HybridPipelineAdapter:
    ...
    The project pipeline is imported lazily. Missing optional dependencies or a
    deliberately absent ``src`` package produce a structured skipped result so
    the review/analytical branch can still run.
    """
    ...
    async def __call__(self, context: MarketContext) -> dict[str, Any]:
        ...
        execution_policy = context.metadata.get("pipeline_execution_policy", {})
        if isinstance(execution_policy, dict) and execution_policy.get("pipeline_run_allowed") is False:
            return self._pipeline_skipped_result(
                context,
                tickers,
                timeframes,
                reason="blocked_by_pipeline_execution_policy",
                extra={"pipeline_execution_policy": execution_policy},
            )
        ...
        if self._src_unavailable:
            return self._pipeline_skipped_result(
                ...
                tickers,
                timeframes,
                reason=self._src_unavailable_reason or "src_pipeline_unavailable",
            )
        ...
        stages_to_run = self._resolve_allowed_stages(context)
        try:
            if self.mode == "local":
            ...
            if not self.degrade_on_unavailable:
                raise
            ...
            return self._pipeline_skipped_result(
                ...
                tickers,
                timeframes,
                reason=self._src_unavailable_reason or repr(exc),
            )

        normalized = self._normalize_result(result, tickers, timeframes)
        normalized["requested_stages"] = stages_to_run
    ...
    def _mark_unavailable(self, exc: Exception) -> None:
        ...
        self._src_unavailable_reason = (
            f"src pipeline unavailable: {type(exc).__name__}: {exc}"
        )
    ...
    async def _execute_full(
        ...
        return await PipelineExecutor.execute_full_mode(
            orchestrator,
            tickers,
            timeframes,
        )
    ...
    def _resolve_allowed_stages(self, context: MarketContext) -> list[int] | None:
        policy = context.metadata.get("pipeline_execution_policy", {})
        policy_stages = policy.get("allowed_stages") if isinstance(policy, dict) else None
        if not policy_stages:
            return list(self.stages_to_run) if self.stages_to_run is not None else None
        normalized_policy = {int(stage) for stage in policy_stages}
        if self.stages_to_run is None:
            return sorted(normalized_policy)
        return [stage for stage in self.stages_to_run if stage in normalized_policy]
    ...
    def _pipeline_skipped_result(
        ...
        *,
        ...
        normalized["dean_os_review_contract"] = self._build_review_contract(normalized)
        context.as_of = context.as_of or self._resolve_context_as_of(normalized)
        self._attach_metric_snapshot(context, normalized)
        context.pipeline_result.update(normalized)
    ...
    def _enrich_context(self, context: MarketContext, result: dict[str, Any]) -> None:
        ...
        self._attach_metric_snapshot(context, result)
        context.pipeline_result.update(result)

    def _attach_metric_snapshot(
        self,
        context: MarketContext,
        result: dict[str, Any],
    ) -> None:
        snapshot = self._metric_normalizer.from_pipeline_result(
            result,
            as_of=context.as_of,
        ...
        result["dean_os_pipeline_metric_snapshot"] = payload
        context.metadata["pipeline_metric_snapshot"] = payload
        # Compatibility projection for existing pipeline agents. New modules
        # should read the versioned snapshot directly.
        context.metadata.update(snapshot.to_agent_metadata())
