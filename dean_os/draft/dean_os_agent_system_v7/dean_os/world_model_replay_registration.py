...
from dean_os.draft.dean_os_agent_system_v7.dean_os.outcome_tracker import OutcomeTracker, REPLAY_INTERVALS
...
def _bridge_issues(
    ...
    invalid_status = [
        str(task.get("task_id") or index)
        for index, task in enumerate(tasks)
        if task.get("registration_status") != "candidate_pending_manual_review"
    ]
    if invalid_status:
        issues.append(
        ...
        plan.append(
            {
                ...
                "source": source,
                "directions": {interval: direction for interval in REPLAY_INTERVALS},
                ...
                "outcome_tracker_semantics": (
                    "OutcomeTracker stores fixed-horizon directional predictions. "
                    "World-model replay hypotheses may be non-directional, so neutral "
                    "projection is used unless an explicit direction is present."
                ),
    ...
    unsupported_horizons = sorted(
        {
            ...
            for item in plan
            if int(item.get("horizon_days") or 0) not in REPLAY_INTERVALS
        }
    )
    if unsupported_horizons:
        warnings.append(
            "outcome_tracker_uses_fixed_intervals_not_exact_task_horizons:"
            + ",".join(str(item) for item in unsupported_horizons)
        )
    return warnings
