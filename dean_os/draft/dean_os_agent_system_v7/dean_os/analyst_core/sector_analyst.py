...
from dean_os.analyst_core.schemas import EvidenceGap, HypothesisLedgerEntry, utc_now_iso
    ...
    def run(
        ...
        *,
        ...
        packet = AnalysisPacket(
            packet_id=f"packet_{self.domain_id}_{utc_now_iso()}",
            as_of_date=as_of,
            source_packet_ids=[e.evidence_id for e in evidence[:10]],
            event_records=event_records,
            entity_links=entity_links,
        )

        # ── Step 3: Lens Pipeline ────────────────────────────────────────
        # Skip lens pipeline when there is no evidence — no events to classify.
    ...
    def run_from_evidence(
        ...
        *,
        ...
        packet = AnalysisPacket(
            packet_id=f"packet_{self.domain_id}_{utc_now_iso()}",
            as_of_date=as_of,
            source_packet_ids=[e.evidence_id for e in normalized[:10]],
            event_records=event_records,
            entity_links=entity_links,
        )

        # Skip lens pipeline when there is no evidence
        if not normalized:
            enriched_packet = packet
            delta_trail = []
        else:
