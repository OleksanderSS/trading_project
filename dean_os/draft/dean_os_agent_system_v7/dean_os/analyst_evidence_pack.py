...
class AnalystEvidencePackRunner:
    ...
    def run(
        ...
        sector_keywords = _normalize_strings(sector_keywords or [])
...
def render_analyst_evidence_pack_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Sector keywords: {', '.join(inputs.get('sector_keywords', [])) or 'none'}",
...
def _document_from_row(
    ...
    if sector_keywords and not _matches_keywords(f"{title} {text}", sector_keywords):
        return None
