...
class DataQualityAgent(BaseAgent):
    ...
    async def run(self, context: MarketContext) -> PipelineReport:
        ...
        analysis_only = context.metadata.get("execution_intent") == "analysis_only"
        if metrics["frame_count"] == 0:
            # Missing frames are a hard block only for an execution-capable path.
            # Agent-only and stages 0-3 analysis may continue as review-only.
            if context.phase == "pre_trade" and not analysis_only:
