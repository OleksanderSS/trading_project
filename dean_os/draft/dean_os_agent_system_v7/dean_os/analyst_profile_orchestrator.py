...
from dean_os.draft.dean_os_agent_system_v7.dean_os.agent_lab import AgentLabRunner
from dean_os.agents.domain_research import MacroPolicyAgent, NewsCatalystAgent, SectorCycleAgent, ValueScreeningAgent
from dean_os.draft.dean_os_agent_system_v7.dean_os.analyst_evidence_pack import documents_from_evidence_pack
from dean_os.draft.dean_os_agent_system_v7.dean_os.review import AgentReviewBuilder
...
def _domain_context(
    ...
    macro_docs = [
        ...
        or document.source_type == "report"
    ]
    context_as_of = (
        evidence_pack.get("inputs", {}).get("as_of")
        or evidence_pack.get("created_at")
    )
    if evidence_pack.get("inputs", {}).get("as_of"):
        for document in documents:
    ...
    return MarketContext(
        ...
        metadata={
            "evidence_pack_run_id": evidence_pack.get("run_id"),
            "source_inventory": {
                "macro_document_count": len(macro_docs),
                "sector_document_count": len(
                    [
                        document
                        for document in documents
                        if set(document.sectors).intersection(sectors)
                    ]
                ),
                "evidence_status": (
                    "document_inventory_not_structured_context_evidence"
                ),
            },
        },
    )
