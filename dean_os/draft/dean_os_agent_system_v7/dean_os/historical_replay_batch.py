...
def resolve_as_of_dates(
    ...
    if as_of_dates:
        return [parse_datetime(value).isoformat() for value in as_of_dates[:max_runs]]
    try:
        import pandas as pd
    ...
    frame = prepare_market_frame(
        pd=pd,
        frame=read_market_frame(pd, path),
        close_col=close_col,
        datetime_col=datetime_col,
    )
    if frame.empty:
    ...
    start = parse_datetime(start_as_of) if start_as_of else min_date + timedelta(days=lookback_days)
    end = parse_datetime(end_as_of) if end_as_of else max_date - timedelta(days=max(horizons))
    if start > end:
        raise ValueError(
