...
class KillSwitchCalculator:
    ...
    def calculate_portfolio_metrics(self,
        ...
        try:
            ...
            # --- Value at Risk (Historical Simulation, 95%) ---
            # Use the percentile method — losses at the 5th percentile of the
            # return distribution.  np.var() returns variance (return²) which
            # is 2-3 orders of magnitude smaller than the thresholds in
            # KillSwitchConfig.RISK_LEVELS (0.15–0.40), making those checks
            # permanently dead.  Percentile-based VaR is on the same 0–1
            # scale as the configured thresholds.
            returns_arr = np.array(portfolio_returns)
            tail_percentile = 5.0          # 1 - 0.95 confidence level
            daily_var = float(max(0.0, -np.percentile(returns_arr, tail_percentile)))

            portfolio_volatility = float(np.std(returns_arr)) * np.sqrt(252)
            ...
            return {
                ...
                'daily_var': daily_var,          # Historical-Simulation VaR 95%, scale 0-1
    ...
    def determine_risk_level(self,
        ...
        portfolio_var_threshold values in RISK_LEVELS (0.15–0.40) are
        calibrated against the Historical-Simulation VaR (0-1 scale, e.g.
        0.15 = 15% 1-day tail loss at 95% confidence).
        """
        ...
        for level in levels:
            ...
            # Check portfolio VaR (Historical Simulation, 95%, scale 0-1)
    ...
    def check_emergency_triggers(self, risk_analysis: dict[str, Any]) -> dict[str, Any]:
        ...
        # daily_var is Historical-Simulation VaR at 95% confidence (scale 0-1).
        # 0.30 means a single-day tail loss of 30% of portfolio value — emergency level.
