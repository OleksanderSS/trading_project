    ...
    def _pre_filter_data(self, features_df: pd.DataFrame, target_series: pd.Series, regime_context_id: str) -> pd.DataFrame | None:
        ...
        if target_cols_present:
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"[{regime_context_id}] Dropping {len(target_cols_present)} "
                    f"target-like column(s) before feature selection: "
                    f"{target_cols_present[:5]}{'...' if len(target_cols_present) > 5 else ''}"
                )
            features_df = features_df.drop(columns=target_cols_present, errors='ignore')
