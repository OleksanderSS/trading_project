...
Moved from src/integration/ and cleaned of synthetic fallback logic.
"""
...
class EnsembleSelector:
    ...
    def _score(
        ...
        if ctx.computational_resources == "low" and name in ("live_adaptive", "consensus_engine"):
            score -= 0.15
        if ctx.computational_resources == "low" and "simple" in cfg["strengths"]:
            score += 0.20
        if ctx.latency_requirement == "high" and "fast" in cfg["strengths"]:
            score += 0.20
    ...
    @staticmethod
    def _create_simple_average_ensemble():
        """Return a lightweight averaging callable for prediction dictionaries."""
        import numpy as np

        def simple_average(predictions: dict[str, Any]) -> np.ndarray:
            if not predictions:
                return np.array([])
    ...
    @staticmethod
    def _create_weighted_average_ensemble():
        """Return a lightweight weighted-average callable."""
        import numpy as np
        ...
        def weighted_average(
            ...
            weighted_sum = np.zeros_like(first_prediction, dtype=float)
            total_weight = 0.0
            for model_name, prediction in predictions.items():
                weight = weights.get(model_name, 0.0)
                weighted_sum += weight * np.asarray(prediction, dtype=float)
                total_weight += weight
            if total_weight <= 0:
                return np.mean(np.stack([np.asarray(p, dtype=float)
                    for p in predictions.values()]), axis=0)
