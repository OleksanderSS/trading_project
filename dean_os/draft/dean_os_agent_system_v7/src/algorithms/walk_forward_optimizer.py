...
class WalkForwardOptimizer(PerformanceMetricsMixin):
    ...
    def _select_best_params(
        ...
        # FIX: Evaluate each candidate with its own params
        def evaluate_candidate(params):
            # Тут передбачається, що _evaluate_parameters може приймати параметри
            # або що optimization_func вже це робить.
            # Оскільки _evaluate_parameters зараз не приймає params,
            # нам потрібен допоміжний метод.
            return self._evaluate_with_params(data, params).get(metric, 0.0)

        return max(candidates, key=evaluate_candidate)

    def _evaluate_with_params(self, data: pd.DataFrame, params: dict[str, Any]) -> dict[str, float]:
        """Оцінка параметрів (тимчасова реалізація, поки не інтегровано в _evaluate_parameters)."""
        # У реальному випадку тут має бути застосування параметрів до моделі
        # і її оцінка на даних. Для цього коду достатньо виклику _evaluate_parameters.
