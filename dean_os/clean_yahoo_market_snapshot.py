    ...
    async def build(
        ...
        *,
        ...
        if max_download_attempts is not None:
            ...
            yahoo_config["max_retries"] = int(max_download_attempts)
            yahoo_config["retry_delay"] = 0
        ...
        payload: dict[str, Any] = {
            ...
            "inputs": {
                ...
                "max_download_attempts": int(
                    yahoo_config.get("max_retries", 3)
                ),
...
def load_verified_clean_yahoo_market_snapshot(
    manifest_path: str | Path,
) -> dict[str, Any]:
    """Verify a saved clean snapshot manifest and its immutable Parquet."""

    path = Path(manifest_path).resolve()
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("clean Yahoo manifest must be a JSON object")
    if payload.get("contract") != CLEAN_YAHOO_MARKET_SNAPSHOT_CONTRACT:
        raise ValueError("unsupported clean Yahoo snapshot contract")
    if payload.get("mode") != "clean_yahoo_market_snapshot":
        raise ValueError("unsupported clean Yahoo snapshot mode")
    summary = payload.get("summary") or {}
    safety = payload.get("safety") or {}
    if summary.get("status") != "clean_market_snapshot_validated":
        raise ValueError("clean Yahoo snapshot is not validated")
    if summary.get("source_gate_issues"):
        raise ValueError("clean Yahoo snapshot has source-gate issues")
    for key in (
        "source_ticker_validated_before_relabel",
        "cross_identity_exact_ohlcv_gate",
        "cadence_gate",
        "finite_ohlcv_gate",
    ):
        if safety.get(key) is not True:
            raise ValueError(f"clean Yahoo required safety flag invalid: {key}")
    for key in (
        "database_write_performed",
        "legacy_artifact_write_performed",
        "learning_write_performed",
        "broker_access_performed",
    ):
        if safety.get(key) is not False:
            raise ValueError(f"clean Yahoo forbidden safety flag invalid: {key}")

    snapshot = payload.get("snapshot") or {}
    snapshot_path = Path(str(snapshot.get("path") or "")).resolve()
    if snapshot.get("format") != "parquet" or not snapshot_path.is_file():
        raise ValueError("clean Yahoo immutable Parquet is missing")
    ...
    if missing:
        raise ValueError(
            "clean Yahoo Parquet schema missing: " + ",".join(missing)
        )
    timestamps = pd.to_datetime(frame["datetime"], errors="coerce", utc=True)
    if timestamps.isna().any():
        raise ValueError("clean Yahoo Parquet contains invalid timestamps")
    actual_frame_sha = _frame_sha256(frame)
    if (
        actual_frame_sha != snapshot.get("sha256")
        or actual_frame_sha != summary.get("snapshot_sha256")
    ):
        raise ValueError("clean Yahoo snapshot frame hash mismatch")
    tickers = sorted(frame["ticker"].astype(str).str.upper().unique().tolist())
    intervals = sorted(frame["interval"].astype(str).str.lower().unique().tolist())
    declared_tickers = _normalize_tickers((payload.get("inputs") or {}).get("tickers") or [])
    if tickers != declared_tickers:
        raise ValueError("clean Yahoo snapshot ticker inventory mismatch")
    if (
        int(summary.get("row_count", -1)) != len(frame)
        or int(summary.get("ticker_count", -1)) != len(tickers)
        or int(summary.get("timeframe_count", -1)) != len(intervals)
    ):
        raise ValueError("clean Yahoo snapshot summary mismatch")
    if _lane_summaries(frame) != list(payload.get("lanes") or []):
        raise ValueError("clean Yahoo snapshot lane inventory mismatch")
    return {
        "manifest_path": str(path),
        "manifest_sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
        "snapshot_path": str(snapshot_path),
        "snapshot_file_sha256": hashlib.sha256(snapshot_path.read_bytes()).hexdigest(),
        "snapshot_frame_sha256": actual_frame_sha,
        "tickers": tickers,
        "timeframes": intervals,
        "end_date": str((payload.get("inputs") or {}).get("end_date") or ""),
        "frame": frame,
    }
