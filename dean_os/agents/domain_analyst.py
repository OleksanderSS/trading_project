...
class DomainAnalystAgent(BaseAgent):
    ...
    async def run(self, context: MarketContext) -> PipelineReport:
        ...
        try:
            ...
            hypothesis_projection = _load_prior_hypotheses(
                self.config,
                domain_id=domain_id,
                as_of=as_of,
            )
            ...
            report = _input_gap_report(
                ...
                domain_id,
                (
                    "configured runtime artifact was rejected or hypothesis "
                    f"journal was invalid: {exc}"
                ),
            )
            report.input_hash = self.context_hash(context)
            report.config_hash = sha256_json(self.config)
        ...
        report: SectorReport = analyst.run(
            ...
            prior_hypotheses=hypothesis_projection["active_hypotheses"],
        )
        ...
        pipeline_report = _report_to_pipeline(
            ...
            report,
            domain_id,
            prior_hypothesis_count=hypothesis_projection[
...
def _report_to_pipeline(
    ...
    *,
    ...
    audit = dict(serialized_report.get("audit") or {})
    proposals = list(report.hypothesis_review_proposals)
    receipt_identity = {
        "domain_id": domain_id,
        "as_of": report.as_of,
        "analysis_input_sha256": audit.get("analysis_input_sha256"),
        "analysis_output_sha256": audit.get("analysis_output_sha256"),
        "delta_sha256s": [
            str(item.get("delta_sha256"))
            for item in audit.get("delta_trail", [])
        ],
        "hypothesis_ids": [item.hypothesis_id for item in report.hypotheses],
        "proposal_ids": [
            str(item.get("proposal_id")) for item in proposals
        ],
    }
    reasoning_receipt = {
        "contract": REASONING_RECEIPT_CONTRACT,
        "receipt_id": "reasoning_receipt_"
        + sha256_json(receipt_identity)[:24],
        **receipt_identity,
        "source_evidence_ids": list(audit.get("source_evidence_ids") or []),
        "review_only": True,
        "hypothesis_status_change_performed": False,
    ...
    return PipelineReport(
        ...
        metrics_snapshot={
            ...
            "hypothesis_review_proposal_count": len(proposals),
            ...
            "hypothesis_review_proposals": proposals,
            "reasoning_receipt": reasoning_receipt,
            "prior_hypothesis_count": prior_hypothesis_count,
            "evidence_gaps": [
                item.model_dump(mode="json")
                if hasattr(item, "model_dump")
                else item
            ...
            "economy_regime_brief": getattr(report, "economy_regime_brief", None),
        },
    )
...
def _load_configured_evidence(
    ...
    *,
    ...
    producer_paths = config.get("producer_artifact_paths") or {
        "news": config.get("news_artifact_path"),
        "macro": config.get("macro_artifact_path"),
        "sector_market": config.get("sector_market_artifact_path"),
        "policy": config.get("policy_artifact_path"),
        "fundamental": config.get("fundamental_artifact_path"),
    }
    # remove None values
    producer_paths = {k: v for k, v in producer_paths.items() if v}
...
def _load_prior_hypotheses(
    config: dict[str, Any],
    *,
    domain_id: str,
    as_of: str,
) -> dict[str, Any]:
    journal_path = str(config.get("hypothesis_journal_path") or "").strip()
    if not journal_path:
        return {"active_hypotheses": [], "active_hypothesis_count": 0}
    return project_active_hypotheses(
        journal_path,
        domain_id=domain_id,
        as_of=as_of,
    )
