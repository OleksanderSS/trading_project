...
class SavedSectorMarketEvidenceProducer:
    ...
    def build(
        ...
        *,
        ...
        requested = sorted(
            {
                ...
                for ticker in (sector_tickers or [])
                if str(ticker).strip()
            }
        )
        ...
        if not requested:
            raise ValueError(
                "sector_tickers must be explicitly supplied from the domain profile"
            )
        if not benchmark:
            raise ValueError(
                "benchmark must be explicitly supplied from the domain profile"
            )
        run_id = _run_id()
