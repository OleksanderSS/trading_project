...
def test_derived_forward_direction_preserves_unavailable_tail_targets():
    enricher = object.__new__(DerivedFeaturesEnricher)
    df = pd.DataFrame({"returns": [0.01, 0.02, -0.01, 0.03]})

    enricher._add_forward_targets(
        df,
        "returns",
        {"periods": [2], "include_returns": True, "include_direction": True},
    )

    assert pd.isna(df["target_forward_return_2P"].iloc[-2:]).all()
    assert df["target_forward_direction_2P"].iloc[-2:].isna().all()
