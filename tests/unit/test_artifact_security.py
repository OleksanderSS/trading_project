...
def test_resolve_trusted_artifact_rejects_outside_project():
    with tempfile.TemporaryDirectory() as tmp_dir:
        outside = Path(tmp_dir) / "model.joblib"

        with pytest.raises(ValueError, match="outside trusted roots"):
            resolve_trusted_artifact_path(outside)


def test_resolve_trusted_artifact_allows_explicit_env_root(monkeypatch):
    with tempfile.TemporaryDirectory() as tmp_dir:
...
def test_model_loader_rejects_untrusted_path_before_deserialization(monkeypatch):
    with tempfile.TemporaryDirectory() as tmp_dir:
        outside = Path(tmp_dir) / "model.joblib"
