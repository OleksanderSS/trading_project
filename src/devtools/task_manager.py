...
class TaskManager:
    ...
    def _scan_file_for_todos(self, py_file: Path, project_path: Path) -> List[Dict[str, Any]]:
        """Scans a single Python file for TODO comments."""
        todos_found = []
                ...
                for line_num, line in enumerate(f, 1):
                    ...
                    if match:
                        ...
                        todos_found.append({
                            'type': todo_type,
                            'text': todo_text,
                            'file': py_file.relative_to(project_path),
                            'line': line_num
                        })
    ...
    def _is_duplicate_task(self, todo_type: str, todo_text: str) -> bool:
        """Checks if a TODO already exists as a task."""
        return any(
            task.title.startswith(f"[{todo_type.upper()}]") and 
            todo_text[:60] in task.title 
            for task in self.tasks.values()
        )
    
    def _create_todo_task(self, todo_info: Dict[str, Any]) -> Task:
        """Creates a task from TODO information."""
        todo_type = todo_info['type']
        todo_text = todo_info['text']
        
        task_title = f"[{todo_type.upper()}] {todo_text[:60]}..."
        
        return self.create_task(
            title=task_title,
            description=f"Found in `{todo_info['file']}` at line {todo_info['line']}.\n\nFull text: {todo_text}",
    ...
    def consolidate_codebase_todos(self, project_root: str = ".") -> List[Task]:
        ...
        for py_file in project_path.rglob("*.py"):
            todos_found = self._scan_file_for_todos(py_file, project_path)
            
            for todo_info in todos_found:
                if self._is_duplicate_task(todo_info['type'], todo_info['text']):
                    continue
                
                task = self._create_todo_task(todo_info)
