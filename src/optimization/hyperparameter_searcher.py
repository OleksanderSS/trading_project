...
class HyperparameterSearcher:
    ...
    def _optimize_grid_search(self, x_train, y_train, x_val, y_val, param_space, metric_func, model_type='mlp'):
        ...
        if len(combinations) > self.n_trials:
            selected_indices = rng.choice(len(combinations), self.n_trials, replace=False)
            combinations = [combinations[idx] for idx in selected_indices]
    ...
    async def _evaluate_mlp_params(self, X_train, y_train, x_val, y_val, params, metric_func=None):
        """Evaluate MLP with given parameters using real model training."""
        from src.models.neural.mlp import SimpleMLP
        
        # Instantiate model with params
        model = SimpleMLP(input_size=X_train.shape[1], **params)
    ...
    async def _evaluate_lstm_params(self, X_train, y_train, x_val, y_val, params, metric_func=None):
        """Evaluate LSTM with given parameters using real model training."""
        from src.models.neural.lstm import SimpleLSTM
        
        # Instantiate model with params
        model = SimpleLSTM(input_size=X_train.shape[1], **params)
        model.fit(X_train, y_train)
