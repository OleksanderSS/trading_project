...
class AdaptiveConfidenceCalibrator:
    ...
    def __init__(self, logger=None, window_size=500, decay_rate=0.95):
        ...
        # Primary models (if sklearn available)
        if SKLEARN_AVAILABLE:
            self.isotonic_model = IsotonicRegression(out_of_bounds="clip")
            self.platt_model = LogisticRegression(solver="lbfgs", random_state=42)
        else:
        ...
        # Simple fallback calibration (when sklearn unavailable)
        self.simple_calibration_map = {}  # {bin: avg_accuracy}
    ...
    def calibrate(self, raw_confidence):
        ...
        """
        # Fallback mode if sklearn not available
        if not SKLEARN_AVAILABLE:
    ...
    def update_with_outcome(self, raw_confidence, actual_outcome):
        ...
        try:
            ...
            if len(self.calibration_history) % 50 == 0:
                ...
                if (
                    self.distribution_shift_detected
                    or len(self.calibration_history) % 200 == 0
                ):
    ...
    def _execute_isotonic_training(self, raw_confs, outcomes, weights):
        ...
        try:
            # Isotonic doesn't support weights directly, use bootstrap sampling
            n_samples = min(len(raw_confs), 200)
            sample_indices = rng.choice(
                len(raw_confs), size=n_samples, p=weights / weights.sum()
            )

            self.isotonic_model.fit(raw_confs[sample_indices], outcomes[sample_indices])
    ...
    def _compute_metrics(self):
        ...
        try:
            ...
            calibrated = np.array(
                [e["calibrated_confidence"] for e in self.calibration_history]
            )
            outcomes = np.array([e["actual_outcome"] for e in self.calibration_history])
    ...
    def _execute_simple_training(self, raw_confs, outcomes, weights):
        ...
        try:
            ...
            for bin_idx in range(10):
                ...
                if np.sum(mask) > 0:
                    # Weighted average accuracy for this bin
                    bin_accuracy = np.average(outcomes[mask], weights=weights[mask])
