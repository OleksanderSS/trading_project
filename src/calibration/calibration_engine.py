...
class CalibrationEngine:
    ...
    def __init__(
        ...
        # Динамічне отримання шляхів
        paths = self.config_manager.get_config('paths', {})
        self.real_data_path = Path(paths.get('trading_db', "data/duckdb/trading.db"))
        self.synthetic_data_path = Path(paths.get('synthetic_data', "data/synthetic/"))
    ...
    def load_real_data(self, test_ticker: str | None = None) -> dict[str, pd.DataFrame]:
        """
    ...
    def load_synthetic_scenarios(self) -> dict[str, list[dict[str, Any]]]:
        ...
        try:
            ...
            typical_files = list(
                self.synthetic_data_path.glob("synthetic_typical_scenarios_*.json")
            )
            ...
            shock_files = list(
                self.synthetic_data_path.glob("synthetic_shock_scenarios_*.json")
            )
            ...
            context_files = list(
                self.synthetic_data_path.glob("synthetic_context_scenarios_*.json")
            )
    ...
    def define_hyperparameter_space(self, trial) -> dict[str, Any]:
        ...
        # Define RL hyperparameters (used in actual DEAN training)
        hyperparams = {
            # Learning rates
        ...
        # Add proxy model hyperparameters for evaluation (RandomForest as DEAN substitute)
        hyperparams["actor_n_estimators"] = trial.suggest_int(
            "actor_n_estimators", 50, 200
        )
        hyperparams["actor_max_depth"] = trial.suggest_int("actor_max_depth", 5, 20)
    ...
    def _evaluate_on_synthetic(
        self, model: Any, synthetic_scenarios: dict[str, list[dict[str, Any]]]
    ) -> float:
    ...
    def _calculate_sharpe_ratio(self, y_true: np.ndarray, y_pred: np.ndarray) -> float:
        ...
        try:
            # ✅ Corrected critical bug: calculate realized trade returns (direction of prediction * actual return)
            # instead of prediction residuals (y_pred - y_true) which penalized perfect predictions and rewarded errors.
    ...
    def run_calibration(
        self, test_ticker: str | None = None, test_target: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        study = optuna.create_study(
            direction="maximize", study_name=f"dean_calibration_{self.batch_name}"
        )
