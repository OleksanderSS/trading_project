...
class BiasDetector:
    ...
    def _future_returns(prices: pd.DataFrame, lag_periods: int) -> pd.DataFrame:
        """
        Calculate future returns from prices.
        Always returns returns (even if NaN) to avoid lookahead bias from using future prices.
        """
        ...
        # Always return returns, even if all NaN - never fall back to future prices
        # Using future prices would introduce lookahead bias
        return returns
