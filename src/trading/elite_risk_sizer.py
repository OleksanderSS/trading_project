...
class EliteRiskSizer:
    ...
    def compute_optimal_position_size(self,
                                     ...
                                     correlation_matrix: dict[str, Any] | None = None) -> tuple[float, dict[str, Any]]:
        """
        ...
        """
        # Prior win_rate and win_loss_ratio derived from signal confidence & magnitude
        win_rate = 0.55 if confidence > 0.6 else 0.51
        win_loss_ratio = 1.8 if abs(prediction) > 0.02 else 1.5

        # ✅ Bayesian shrinkage: Update prior metrics using authentic realized historical returns posterior
        if ticker in self.historical_returns and not self.historical_returns[ticker].empty:
            ticker_ret = self.historical_returns[ticker]
            pos_ret = ticker_ret[ticker_ret > 0]
            neg_ret = ticker_ret[ticker_ret < 0]
            if len(ticker_ret) > 10:
                emp_win_rate = len(pos_ret) / len(ticker_ret)
                emp_ratio = float(pos_ret.mean() / abs(neg_ret.mean())) if len(pos_ret) > 0 and len(neg_ret) > 0 else 1.5
                
                # Shrink prior toward empirical evidence (30% weight to empirical realized stats)
                win_rate = 0.7 * win_rate + 0.3 * emp_win_rate
                win_loss_ratio = 0.7 * win_loss_ratio + 0.3 * emp_ratio
                self.logger.info(f"[RISK SIZER] Bayesian empirical adjustments active for {ticker}: win_rate={win_rate:.3f}, ratio={win_loss_ratio:.3f}")
