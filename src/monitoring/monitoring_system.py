...
class BaseMonitor:
    ...
    def generate_report(self) ->dict[str, Any]:
        ...
        return {'monitor_name': self.name, 'timestamp': datetime.now().
            isoformat(), 'metrics': self.metrics, 'alerts': [(alert.
            __dict__ if hasattr(alert, '__dict__') else alert) for alert in
            self.alerts[-10:]], 'status': 'running' if self.is_running else
            'stopped'}
...
class SystemHealthMonitor(BaseMonitor):
    ...
    def collect_metrics(self) ->dict[str, Any]:
        ...
        try:
            ...
            metrics = {'cpu_percent': cpu_percent, 'memory_percent':
                memory_percent, 'memory_used_gb': memory_used_gb,
                'memory_available_gb': memory_available_gb, 'disk_percent':
                disk_percent, 'disk_used_gb': disk_used_gb, 'disk_free_gb':
                disk_free_gb, 'network_bytes_sent': bytes_sent,
                'network_bytes_recv': bytes_recv, 'process_count':
    ...
    def check_thresholds(self):
        ...
        if self.metrics.get('cpu_percent', 0) > self.cpu_threshold:
            self._create_alert(
                ...
                alertseverity.WARNING, {'cpu_percent': self.metrics[
                'cpu_percent']})
        if self.metrics.get('memory_percent', 0) > self.memory_threshold:
            self._create_alert(
                f"High memory usage: {self.metrics['memory_percent']:.1f}%",
                alertseverity.ERROR, {'memory_percent': self.metrics[
                'memory_percent']})
        if self.metrics.get('disk_percent', 0) > self.disk_threshold:
            self._create_alert(
                f"Low disk space: {self.metrics['disk_percent']:.1f}% used",
                alertseverity.CRITICAL, {'disk_percent': self.metrics[
                'disk_percent']})

    def _create_alert(self, message: str, severity: alertseverity, details:
        dict[str, Any]):
        """Internal helper to register an alert."""
        alert = {'id': f'{self.name}_{int(time.time())}', 'monitor': self.
            name, 'message': message, 'severity': severity.value, 'status':
...
class ModelPerformanceMonitor(BaseMonitor):
    ...
    def update_model_metrics(self, model_name: str, metrics: dict[str, Any]):
        ...
        if abs(current_accuracy - baseline_accuracy) > self.drift_threshold:
            self._create_drift_alert(model_name, current_accuracy,
                baseline_accuracy)

    def _create_drift_alert(self, model_name: str, current_acc: float,
        baseline_acc: float):
        """Creates an alert when model drift exceeds thresholds."""
        alert = {'id': f'drift_{model_name}_{int(time.time())}', 'monitor':
            self.name, 'message':
            f'Model drift detected for {model_name}: accuracy {baseline_acc:.3f} -> {current_acc:.3f}'
            , 'severity': alertseverity.WARNING.value, 'status':
            alertstatus.ACTIVE.value, 'timestamp': datetime.now().isoformat
            (), 'details': {'model_name': model_name, 'baseline_accuracy':
            baseline_acc, 'current_accuracy': current_acc, 'drift': abs(
...
class DataQualityMonitor(BaseMonitor):
    ...
    def __init__(self, config: (dict[str, Any] | None)=None):
        ...
        self.outlier_threshold = self.config.get('outlier_threshold', 0.1)
        self.consistency_threshold = self.config.get('consistency_threshold',
            0.95)
    ...
    def collect_metrics(self) ->dict[str, Any]:
        ...
        try:
            metrics = {'total_sources': len(self.data_sources),
                'sources_with_issues': sum(1 for s in self.data_sources.
                values() if s.get('has_issues', False)),
                'average_completeness': np.mean([s.get('completeness', 1.0) for
                s in self.data_sources.values()]) if self.data_sources else
                1.0, 'total_missing_values': sum(s.get('missing_count', 0) for
    ...
    def update_data_quality(self, source_name: str, data_quality_report:
        ...
        if completeness < 1 - self.missing_threshold:
            self._create_quality_alert(
                ...
                alertseverity.WARNING, {'source': source_name,
                'completeness': completeness})

    def _create_quality_alert(self, message: str, severity: alertseverity,
...
class AlertManager:
    ...
    def __init__(self, config: (dict[str, Any] | None)=None):
        ...
        self.alert_channels: list[str] = self.config.get('channels', ['log'])
    ...
    def get_active_alerts(self, severity: (alertseverity | None)=None) ->list[
        dict[str, Any]]:
...
class MonitoringDashboard:
    ...
    def __init__(self, monitors: list[BaseMonitor], alert_manager:
        AlertManager, config: (dict[str, Any] | None)=None):
    ...
    def generate_dashboard_data(self) ->dict[str, Any]:
        ...
        try:
            ...
            dashboard_data: dict[str, Any] = {'timestamp': datetime.now().
                isoformat(), 'system_status': 'healthy', 'monitors':
                monitors_data, 'alerts': {'active': self.alert_manager.
                get_active_alerts(), 'by_severity': self.
                _group_alerts_by_severity(), 'recent': self.
                _get_recent_alerts(24)}, 'summary': self._generate_summary()}
            for monitor in self.monitors:
                ...
                if monitor.alerts and any(a.get('severity') in ['error',
    ...
    def _group_alerts_by_severity(self) ->dict[str, int]:
        ...
        for severity in alertseverity:
            grouped[severity.value] = sum(1 for a in alerts if a['severity'
                ] == severity.value)
        return grouped
...
class MonitoringSystem:
    ...
    def __init__(self, config: (dict[str, Any] | None)=None):
        ...
        self.system_monitor = SystemHealthMonitor(self.config.get(
            'system_health', {}))
        self.model_monitor = ModelPerformanceMonitor(self.config.get(
            'model_performance', {}))
        self.data_monitor = DataQualityMonitor(self.config.get(
            'data_quality', {}))
        self.alert_manager = AlertManager(self.config.get('alerts', {}))
        self.monitors = [self.system_monitor, self.model_monitor, self.
            data_monitor]
        self.dashboard = MonitoringDashboard(self.monitors, self.
            alert_manager, self.config.get('dashboard', {}))
    ...
    def get_health_report(self) ->dict[str, Any]:
        ...
        return {'system_status': 'running' if self.is_running else
            'stopped', 'monitors': {m.name: m.is_running for m in self.
            monitors}, 'active_alerts': len(self.alert_manager.
    ...
    def update_data_quality(self, source_name: str, quality_report: dict[
        str, Any]):
