...
class ResourceMonitor:
    ...
    def start_monitoring(self, interval: int=5):
        ...
        if self.monitor_thread is None or not self.monitor_thread.is_alive():
            ...
            self.monitor_thread = Thread(target=self._monitor_loop, args=(
                interval,), daemon=True)
            self.monitor_thread.start()
            self.logger.info(
                f'Resource monitoring started with interval {interval}с.')
    ...
    def collect_all_metrics(self) ->Dict[str, Any]:
        ...
        futures = {'system': self.executor.submit(self.
            ...
            _collect_disk_metrics), 'processes': self.executor.submit(self.
    ...
    def _check_thresholds(self, metrics: Dict[str, Any]):
        ...
        mem_percent = metrics.get('system', {}).get('memory', {}).get('percent'
            , 0)
        disk_percent = metrics.get('disk', {}).get('usage', {}).get('percent',
            0)
    ...
    def _is_critical_level(self, cpu_percent: float, mem_percent: float,
        disk_percent: float) ->bool:
        """Check if any resource is at critical level."""
        return cpu_percent > self.thresholds['cpu_critical'
            ] or mem_percent > self.thresholds['memory_critical'
            ] or disk_percent > self.thresholds['disk_critical']

    def _is_warning_level(self, cpu_percent: float, mem_percent: float,
        disk_percent: float) ->bool:
        """Check if any resource is at warning level."""
        return cpu_percent > self.thresholds['cpu_warning'
            ] or mem_percent > self.thresholds['memory_warning'
