...
class MonitoringConfig:
    ...
    def _apply_environment_variables(self):
        ...
        env_mappings = {'MONITORING_CPU_THRESHOLD': ('system_health',
            'cpu_threshold'), 'MONITORING_MEMORY_THRESHOLD': (
            'system_health', 'memory_threshold'),
            'MONITORING_DISK_THRESHOLD': ('system_health', 'disk_threshold'
            ), 'MONITORING_COLLECTION_INTERVAL': ('collection_interval',),
            'MONITORING_AUTO_RESOLVE_HOURS': ('alerts',
            'auto_resolve_hours'), 'MONITORING_DASHBOARD_PORT': (
            'dashboard', 'web', 'port'), 'MONITORING_DASHBOARD_HOST': (
            'dashboard', 'web', 'host')}
        for env_var, config_path in env_mappings.items():
            value = os.getenv(env_var)
            if value is not None:
                self._set_nested_config(self.config, config_path, self.
    ...
    def _get_default_config(self) ->Dict[str, Any]:
        return {'collection_interval': 30, 'enabled_monitors': [
            ...
            'accuracy_threshold': 0.7, 'mae_threshold': 0.1,
            'drift_threshold': 0.05, 'performance_window': 100,
            'baseline_period_days': 30, 'enabled_metrics': ['accuracy',
            'mae', 'drift', 'latency']}, 'data_quality': {
            'missing_threshold': 0.05, 'outlier_threshold': 0.1,
            'consistency_threshold': 0.95, 'duplicate_threshold': 0.02,
            'enabled_checks': ['completeness', 'consistency', 'outliers',
            'duplicates']}, 'alerts': {'channels': ['log'],
            'auto_resolve_hours': 24, 'max_alerts_per_hour': 10,
            'severity_levels': ['info', 'warning', 'error', 'critical']},
            'dashboard': {'refresh_interval': 5000, 'history_days': 7,
            'auto_save': True, 'save_interval': 3600, 'save_path':
    ...
    def get_config_for_environment(self, environment: str) ->Dict[str, Any]:
        ...
        if environment == 'development':
            ...
            base_config.update({'collection_interval': 20, 'alerts': {
                'channels': ['log', 'email']}})
        elif environment == 'production':
            base_config.update({'collection_interval': 60, 'alerts': {
...
if __name__ == '__main__':
    ...
    parser.add_argument('--environment', default='development', choices=[
