...
class TimeSeriesValidator:
        ...
        for i in range(window_size, n_samples, step_size):
            ...
            try:
                ...
                fold_metrics.append({
                    ...
                    'r2': float(r2_score(y_val, y_pred)) if len(y_val) >= 2 else 0.0
                })
    ...
    # ✨ NEW: Purged Walk-Forward (from ValidationProtocolsEngine)
    def purged_walk_forward_validation(self, model, X: pd.DataFrame, y: pd.Series,
                                      purge_window: int = 5, embargo_period: int = 10) -> ValidationResult:
        """
        ...
        """
        ps = PurgedTimeSeriesSplit(n_splits=self.n_splits, purge_window=purge_window, embargo_period=embargo_period)
    ...
    # ✨ NEW: Stratified Time Series Split (from SmartCrossValidator)
    def stratified_time_series_split(self, X: pd.DataFrame, y: pd.Series, n_bins: int = 5) -> Generator[tuple[np.ndarray, np.ndarray], None, None]:
        """
    ...
    # ✨ NEW: Comprehensive Validation (from ValidationProtocolsEngine)
    def run_comprehensive_validation(self, data: pd.DataFrame, features: list[str], target: str,
        ...
        # 2. Purged CV
        results["purged_cv"] = self._run_purged_cv(data, features, target, model)

        # 3. Embargo CV
        results["embargo_cv"] = self._run_embargo_cv(data, features, target, model)
    ...
    def _run_purged_cv(self, data: pd.DataFrame, features: list[str], target: str, model: Any) -> ValidationResult:
        """Runs CV with purging."""
        ps = PurgedTimeSeriesSplit(n_splits=self.n_splits, purge_window=5, embargo_period=0)
    ...
    def _run_embargo_cv(self, data: pd.DataFrame, features: list[str], target: str, model: Any) -> ValidationResult:
        """Runs CV with embargo."""
        ps = PurgedTimeSeriesSplit(n_splits=self.n_splits, purge_window=0, embargo_period=10)
    ...
    def _run_consensus_validation(self, data: pd.DataFrame, features: list[str],
        ...
        if len(data) < min_test_size:
            return ValidationResult(ValidationType.CONSENSUS_STABILITY, False, 0.0, {}, ["Data too small"], [], {})
        ...
        try:
            ...
            return ValidationResult(
                ...
                is_valid=agreement >= agreement_threshold,
                confidence=agreement,
                performance_metrics={"agreement_rate": agreement},
                issues_found=[] if agreement >= agreement_threshold else ["Low agreement"],
