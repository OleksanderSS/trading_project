...
class RuntimeConfigLoader:
    ...
    FULL_MODE_MESSAGE = (
        '📊 Режим: full (всі тікери, всі таргети, стандартні епохи)')
    ...
    def _get_batch_name_from_args(self):
        ...
        if '--batch-name' in sys.argv:
            try:
                ...
                logger.warning(f"Failed to parse --batch-name argument: {e}")
        return None
    ...
    def _build_batch_dir_path(self, batch_name):
        ...
        batch_dir = (self.project_path / 'data' / 'colab' / 'accumulated' /
            batch_name)
    ...
    def _print_batch_source(self, runtime_params_path, batch_name):
        ...
        legacy_path = (self.project_path / 'src' / 'config' /
            RUNTIME_PARAMS_FILE)
        ...
        try:
            ...
            with open(config_path) as f:
                ...
                if content:
                    ...
                    print(
                        '⚠️ config.json порожній, використовуємо ПОВНОЦІННІ параметри за замовчуванням'
                        )
                    print(self.FULL_MODE_MESSAGE)
    ...
    def _parse_config_data(self, config_data):
        ...
        if test_mode.get('enabled'):
            ...
            print(
                f'🧪 ТЕСТОВИЙ РЕЖИМ: {self.TEST_TICKER} | {self.TEST_TARGET} | epochs={self.REDUCED_EPOCHS}'
                )
        else:
            print(
                '⚠️ config.json порожній або test_mode disabled, використовуємо ПОВНОЦІННІ параметри'
                )
            print(self.FULL_MODE_MESSAGE)
