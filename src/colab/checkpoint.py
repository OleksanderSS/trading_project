...
def save_checkpoint(params):
    """Save checkpoint for training recovery."""
    from .config import CheckpointParams
...
def load_checkpoint(checkpoint_path, model, optimizer) -> Tuple[int, float]:
    """Load checkpoint for training recovery."""
    if not TORCH_AVAILABLE:
...
def find_latest_checkpoint(checkpoint_dir: Path, ticker: str, target_col: str, m_type: str) -> Optional[Path]:
