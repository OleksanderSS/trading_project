"""Configuration management for Colab training."""
...
class RuntimeConfigLoader:
    """Loading and managing runtime configuration."""
    ...
    def __init__(self, project_path, force_full_mode=False):
        ...
        self.TEST_TICKER: Optional[str] = None
        self.TEST_TARGET: Optional[str] = None
        self.REDUCED_EPOCHS: Optional[int] = None
        self.MAX_ITERATIONS = 100

    def load_runtime_params(self):
        """Load runtime parameters from files."""
    ...
    def _find_config_file(self) -> Optional[Path]:
        """Find path to config.json in batch_dir."""
    ...
    def _get_batch_name_from_args(self) -> Optional[str]:
        """Get batch_name from command line arguments."""
    ...
    def _determine_batch_directory(self, config_path: Optional[Path]) -> Path:
        """Determine batch directory based on config.json."""
        if self.force_full_mode:
    ...
    def _build_batch_dir_path(self, batch_name: str) -> Path:
        """Build path to batch directory."""
        batch_dir = self.project_path / "data" / "colab" / "accumulated" / batch_name
        print(f"🔧 Шукаємо дані в {batch_dir}")
        return batch_dir

    def _load_batch_params(self, batch_dir: Path, config_path: Optional[Path] = None):
        """Load parameters from batch directory."""
    ...
    def _print_force_full_mode(self):
        """Print force full mode message."""
        print("🗑️ FORCE FULL MODE: ігноруємо config.json")
        print(self.FULL_MODE_MESSAGE)

    def _handle_config_in_main_database(self, config_path: Path):
        """Handle config.json in main_database."""
    ...
    def _parse_config_data(self, config_data: Dict):
        ...
        if test_mode.get('enabled'):
            ...
            self.REDUCED_EPOCHS = test_mode.get('epochs', 5)
            self.MAX_ITERATIONS = test_mode.get('max_iterations', 5)
