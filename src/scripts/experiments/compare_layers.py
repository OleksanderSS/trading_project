...
class CompareLayersExperiment(BaseExperiment):
    ...
    def run_single_test(self, test_params: dict) -> dict:
        ...
        try:
            ...
            # Simulate holding strategy for baseline metrics (simplified)
            baseline_returns = np.random.normal(0.0005, 0.01, size=len(actual_returns))
            metrics = FinancialMetricsLibrary.calculate_performance_metrics(baseline_returns)
            
            # Simple simulation for ensemble returns
            ensemble_returns = np.where(ensemble_result > 0, baseline_returns, -baseline_returns)
            ensemble_metrics = FinancialMetricsLibrary.calculate_performance_metrics(ensemble_returns)
            
            if len(ensemble_result) == 0 or actual_returns.empty:
                metrics = {}
