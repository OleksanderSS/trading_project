...
from src.models.adapters.unified_model_adapter import UnifiedModelAdapter
...
from src.pipeline.guards.macro_release_timing_guard import MacroReleaseTimingGuard, get_macro_release_timing_guard
from src.pipeline.guards.safe_feature_combiner import SafeFeatureCombiner, get_safe_feature_combiner
from src.pipeline.guards.temporal_leakage_guard import TemporalLeakageGuard, get_temporal_leakage_guard
from src.pipeline.guards.temporal_target_guard import TemporalTargetGuard, get_temporal_target_guard

# ✅ NEW: Temporal safety guards
from src.pipeline.guards.timeframe_alignment_guard import TimeframeAlignmentGuard, get_timeframe_alignment_guard
...
from src.validation.data_leakage_detector import DataLeakageDetector
...
class FeatureEngineeringStage(BaseStage):
    ...
    """
    def _perform_leakage_audit(self, df: pd.DataFrame, target_col: str):
        """Mandatory leakage audit gate."""
        detector = DataLeakageDetector()
        report = detector.run_comprehensive_audit(df, target_col=target_col)
        
        if report.leakage_detected:
            msg = f"❌ LEAKAGE DETECTED (Severity: {report.severity}): {report.leakage_types}"
            self.logger.critical(msg)
            if report.severity == 'critical':
                raise Exception(msg)
            self.logger.warning(f"Recommendations: {report.recommendations}")
        else:
            self.logger.info("✅ Feature audit passed: No leakage detected.")
    ...
    def __init__(self, config_manager: UnifiedConfigManager, error_handler: ErrorHandler, **kwargs):
        ...
        # ✅ УНІФІКОВАНИЙ АДАПТЕР ФІЧ
        self.integrated_adapter = UnifiedModelAdapter()
        ...
        self.master_features_path = self.output_dir / "features.parquet"
        ...
        self.timeframe_guard: TimeframeAlignmentGuard | None = None
        self.safe_combiner: SafeFeatureCombiner | None = None
        self.temporal_target_guard: TemporalTargetGuard | None = None
        self.temporal_leakage_guard: TemporalLeakageGuard | None = None
        self.macro_guard: MacroReleaseTimingGuard | None = None
    ...
    def _load_runtime_params(self, batch_name: str | None = None) -> dict[str, Any]:
    ...
    def _initialize_guards_based_on_mode(self, mode: str = "full"):
        ...
        # Also initialize guards for prepare mode to ensure proper data processing
        if mode == "prepare":
            self.timeframe_guard = get_timeframe_alignment_guard(strict_mode=False)  # Non-strict for prepare mode
            logger.info(" Prepare mode: Using non-strict timeframe guard for data accumulation")

    def _process_single_timeframe(self, tf: str, df_temp: pd.DataFrame, cleaned_data: dict[str, Any],
                                test_ticker: str | None) -> pd.DataFrame | None:
        ...
        # Enrich per ticker to keep rolling indicators and stateful enrichers isolated.
        if 'ticker' in df_temp.columns and df_temp['ticker'].nunique(dropna=True) > 1:
            enriched_parts = []
            for ticker, ticker_df in df_temp.groupby('ticker', sort=False):
                logger.info(f"Enriching ticker slice {ticker} for timeframe {actual_tf} ({len(ticker_df)} rows)")
                enriched_part = self._enrich_with_cache(ticker_df.copy(), actual_tf, cleaned_data)
                if enriched_part is not None and not enriched_part.empty:
                    enriched_parts.append(enriched_part)
            df_enriched = pd.concat(enriched_parts, ignore_index=True) if enriched_parts else None
        else:
    ...
    def _ensure_enriched_datetime(self, df_enriched: pd.DataFrame, df_temp: pd.DataFrame, timeframe: str) -> pd.DataFrame:
        ...
        if 'datetime' not in df_enriched.columns:
            ...
            if 'datetime' in df_temp.columns:
                ...
                if len(df_enriched) == len(df_temp):
                    ...
                    # ✅ BUG FIX: raise was outside else — now correctly indented inside else block
                    logger.error(f"❌ Length mismatch: enriched={len(df_enriched)}, original={len(df_temp)}")
                    logger.error("   Cannot copy datetime — dropping rows without valid datetime.")
                    raise ValueError(
                        f"Datetime alignment failed for {timeframe}: "
                        f"enriched rows={len(df_enriched)}, original rows={len(df_temp)}. "
                        "Cannot safely assign timestamps."
                    )
            else:
                logger.error(f"❌ Original data has no datetime column for {timeframe}")
                raise ValueError(
                    f"Cannot restore datetime for {timeframe}: original DataFrame has no datetime column."
                )
        else:
        ...
        if df_enriched['datetime'].isna().any():
            ...
            logger.error("   This should not happen - dropping rows with NaT datetime")
    ...
    def _filter_test_ticker(self, df_temp: pd.DataFrame, test_ticker: str | None) -> pd.DataFrame:
    ...
    def _enrich_with_cache(self, df_temp: pd.DataFrame, actual_tf: str,
                          cleaned_data: dict[str, Any]) -> pd.DataFrame | None:
        """Enrich dataframe with feature caching."""
        # ✅ Phase 3 Optimization: Feature caching (60-80% speedup for repeated enrichments)
        import hashlib
        import json
        # Generate cache key from full dataset identity. A timeframe batch can
        # contain many tickers, so using only the first ticker can reuse stale data.
        if df_temp.empty:
            ticker_for_cache = 'UNKNOWN'
            data_hash = 'empty'
        else:
            tickers = sorted(df_temp['ticker'].dropna().astype(str).unique().tolist())
            # FIX: Use all tickers for cache key to prevent ticker loss
            ticker_for_cache = f"MULTI_{len(tickers)}"
            identity_cols = [
                col for col in ('datetime', 'ticker', 'interval', 'timeframe')
                if col in df_temp.columns
            ]
            identity_df = df_temp[identity_cols].copy() if identity_cols else df_temp.index.to_frame(index=False)
            data_hash = hashlib.sha256(
                pd.util.hash_pandas_object(identity_df, index=True).values.tobytes()
            ).hexdigest()[:12]
        ...
        cache_date_key = f"{ticker_for_cache}_{actual_tf}_{data_hash}_{config_hash[:8]}"
        ...
        if df_enriched_tf is not None:
            ...
            # Debug: покажемо, що передається в orchestrator
            logger.info(f"🔍 DEBUG: Passing to orchestrator - cleaned_data keys: {list(cleaned_data.keys())}")
            if 'news' in cleaned_data:
                news_data = cleaned_data['news']
                logger.info(f"🔍 DEBUG: News data shape: {news_data.shape if hasattr(news_data, 'shape') else 'unknown'}")
    ...
    def _enrich_timeframes(self, market_data_raw: dict[str, pd.DataFrame], cleaned_data: dict[str, Any], test_ticker: str | None) -> tuple[dict[str, pd.DataFrame], dict[str, pd.DataFrame]]:
        ...
        logger.info(f"🔍 DEBUG: _enrich_timeframes called with {len(market_data_raw)} timeframes")
        for tf, data in market_data_raw.items():
            if hasattr(data, 'shape'):
                logger.info(f"🔍 DEBUG: Input {tf}: DataFrame with shape {data.shape}")
            elif isinstance(data, dict):
                logger.info(f"🔍 DEBUG: Input {tf}: dict with keys {list(data.keys())}")
                if 'data' in data:
                    logger.info(f"🔍 DEBUG: {tf}['data']: {type(data['data'])} with shape {getattr(data['data'], 'shape', 'unknown')}")
            else:
                logger.info(f"🔍 DEBUG: Input {tf}: {type(data)} with {len(data) if hasattr(data, '__len__') else 'unknown'} items")

        for tf, df_temp in market_data_raw.items():
            df_enriched_tf = self._process_single_timeframe(tf, df_temp, cleaned_data, test_ticker)
            if df_enriched_tf is not None:
                # CRITICAL: Ensure datetime is a column, not index
                if isinstance(df_enriched_tf.index, pd.DatetimeIndex) and 'datetime' not in df_enriched_tf.columns:
                    df_enriched_tf = df_enriched_tf.reset_index()
                    if 'index' in df_enriched_tf.columns:
                        df_enriched_tf = df_enriched_tf.rename(columns={'index': 'datetime'})
                    logger.debug(f"✅ Converted DatetimeIndex to datetime column in main loop for {tf}")
                ...
                if self.temporal_target_guard is not None:
                    # Get targets for this TF from orchestrator
                    t_configs = []
                    if hasattr(self.target_orchestrator, 'target_configs'):
                        t_configs = self.target_orchestrator.target_configs.get(tf, [])
                    elif hasattr(self.target_orchestrator, 'targets'):
                        # Fallback filtering logic
                        t_configs = [t for t in self.target_orchestrator.targets if tf in t.get('name', '')]
                    ...
                    logger.warning("Temporal target guard is None, skipping target generation")
                    continue
        ...
        return enriched_prices, all_targets

    def _build_news_features(self, cleaned_data: dict[str, Any], enriched_prices: dict[str, pd.DataFrame]) -> pd.DataFrame | None:
    ...
    def _process_prepare_mode(self, enriched_prices: dict[str, pd.DataFrame], all_targets: dict[str, pd.DataFrame]) -> dict[str, Any]:
        """Process prepare mode return."""
        ...
        for tf, df in enriched_prices.items():
            ...
            # ✅ Ensure interval column exists for deduplication
        ...
        return {
            ...
            "all_targets_df": targets_df, # ✅ NEW: Add combined targets
    ...
    def execute(self, **kwargs) -> dict[str, Any]:
        """Execute stage with standard interface."""
        import asyncio
    ...
    async def run(self, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            if self.timeframe_guard is None or mode == "prepare":
                self._initialize_guards_based_on_mode(mode)

            # ✅ Initialize with empty dicts to prevent UnboundLocalError
            enriched_prices, all_targets = {}, {}

            # Enrich timeframes
            enriched_prices, all_targets = self._enrich_timeframes(market_data_raw, cleaned_data, test_ticker)
            ...
            news_features_df = self._build_news_features(cleaned_data, enriched_prices)

            # ✅ INT FIX: Removed duplicate parquet save that was here (lines 676-710).
            # The canonical persistence happens later in full-mode with proper datetime+ticker alignment.
            # Keeping the prepare-mode path below intact.

            if mode == "prepare":
                prepare_results = self._process_prepare_mode(enriched_prices, all_targets)
                # Update metadata with news features info
                if news_features_df is not None and not news_features_df.empty:
                    prepare_results["features_metadata"].update({
                        "news_rows": len(news_features_df),
                        "news_columns": len(news_features_df.columns)
                    })
                return prepare_results

            # Full mode
            current_time = pd.Timestamp.now()
            temporal_validation_results: dict[str, Any] = {}
            for tf, df in enriched_prices.items():
                if self.temporal_leakage_guard is not None:
                    validation_result = self.temporal_leakage_guard.validate_rolling_windows(df, current_time, tf)
                else:
                    continue
                temporal_validation_results[tf] = validation_result

            if self.safe_combiner is not None:
                combined_data: tuple[pd.DataFrame, dict[str, Any]] = self.safe_combiner.combine_features_safe(enriched_prices, current_time)
            else:
                combined_data = (pd.concat(enriched_prices.values(), ignore_index=True), {})
            ...
            # ✅ INT FIX: Activate drift_monitor and freshness_monitor for real monitoring
            if not features_df.empty:
                try:
                    drift_report = self.drift_monitor.check(features_df)
                    if drift_report and drift_report.get('has_drift'):
                        drifted = drift_report.get('drifted_features', [])
                        logger.warning(f"⚠️ Feature drift detected in {len(drifted)} features: {drifted[:10]}")
                    else:
                        logger.info("✅ drift_monitor: no significant feature drift detected")
                except Exception as dm_err:
                    logger.debug(f"drift_monitor skipped: {dm_err}")

                try:
                    freshness_report = self.freshness_monitor.check(features_df)
                    if freshness_report and freshness_report.get('stale_features'):
                        stale = freshness_report.get('stale_features', [])
                        logger.warning(f"⚠️ Stale data detected in {len(stale)} features: {stale[:10]}")
                    else:
                        logger.info("✅ freshness_monitor: all features are fresh")
                except Exception as fm_err:
                    logger.debug(f"freshness_monitor skipped: {fm_err}")

                try:
                    regime_report = self.regime_tracker.update(features_df)
                    if regime_report:
                        regime_name = regime_report.get('regime', 'unknown')
                        logger.info(f"✅ regime_tracker: current market regime = {regime_name}")
                except Exception as rt_err:
                    logger.debug(f"regime_tracker skipped: {rt_err}")

            # ✅ CALC FIX: Replace integer-index intersection with datetime+ticker merge for
            # semantically correct features/targets alignment before Parquet save.
            try:
                processed_dir = Path("data/processed/features")
                processed_dir.mkdir(parents=True, exist_ok=True)

                feat_path = processed_dir / "features.parquet"
                target_path = processed_dir / "targets.parquet"

                if not features_df.empty and not targets_df.empty:
                    # Prefer semantic alignment by datetime+ticker keys
                    align_keys = [k for k in ('datetime', 'ticker', 'interval') if k in features_df.columns and k in targets_df.columns]
                    if align_keys:
                        target_cols = [c for c in targets_df.columns if c.startswith('target_')]
                        targets_subset = targets_df[align_keys + target_cols].drop_duplicates(subset=align_keys)
                        features_aligned = features_df.merge(targets_subset, on=align_keys, how='inner', suffixes=('', '_dup'))
                        dup_cols = [c for c in features_aligned.columns if c.endswith('_dup')]
                        if dup_cols:
                            features_aligned.drop(columns=dup_cols, inplace=True)
                        targets_aligned = features_aligned[align_keys + [c for c in features_aligned.columns if c.startswith('target_')]]
                        features_aligned_clean = features_aligned.drop(columns=[c for c in features_aligned.columns if c.startswith('target_')], errors='ignore')
                        features_aligned_clean.to_parquet(feat_path, index=False)
                        targets_aligned.to_parquet(target_path, index=False)
                        logger.info(f"✅ Persisted aligned features ({features_aligned_clean.shape}) and targets ({targets_aligned.shape}) to Parquet via {align_keys} merge.")
                    else:
                        # Fallback: save as-is with a warning
                        logger.warning("⚠️ No common alignment keys found — saving features/targets as-is (alignment NOT guaranteed)")
                        features_df.to_parquet(feat_path, index=False)
                        targets_df.to_parquet(target_path, index=False)
                        logger.info(f"✅ Persisted features ({features_df.shape}) and targets ({targets_df.shape}) to Parquet.")
                elif not features_df.empty:
                    features_df.to_parquet(feat_path, index=False)
                    logger.info(f"✅ Persisted features only ({features_df.shape}) to Parquet (no targets).")


            return {
                "status": "success",
                "enriched_data": features_df,  # Key for compatibility
                "targets_data": targets_df,    # Key for compatibility
                "enriched_prices": enriched_prices,
                "all_targets": all_targets,
                "combined_features": combined_data,
                "temporal_validation": temporal_validation_results,
                "feature_combination": {"status": "success"},
                ...
                "models_metadata": {
                    "feature_models": {
                        ...
                        "news_dataset_builder": "NewsContextDatasetBuilder"
                    },
                    "version": "1.0",
                    "timestamp": pd.Timestamp.now().isoformat()
                }
            ...
            logger.error(f"Error in feature engineering stage: {e}", exc_info=True)
    ...
    def _prepare_macro_data(self, macro_data_raw: pd.DataFrame) -> pd.DataFrame:
        ...
        macro_data = self._pivot_macro_data(macro_data_raw, date_col or 'date')  # type: ignore
    ...
    def _get_date_column(self, df: pd.DataFrame) -> str | None:
    ...
    def _process_ticker_data(self, price_1d: pd.DataFrame, ticker: str) -> pd.DataFrame | None:
    ...
    def _fallback_feature_combination(self, enriched_prices: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
        """Fallback method for combining features when safe_combiner is None."""
        combined_data = {}
        for tf, df in enriched_prices.items():
            if not df.empty:
                combined_data[tf] = df.copy()
        return combined_data

    def _create_events_for_moves(self, ticker: str, significant_moves: pd.DataFrame) -> list[dict[str, Any]]:
