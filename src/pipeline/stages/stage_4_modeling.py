...
class ModelingStage(BaseStage):
    ...
    def __init__(self, config_manager: UnifiedConfigManager, error_handler: ErrorHandler, **kwargs):
        ...
        self.bayesian_optimizer: BayesianOptimizer | None = None
        try:
    ...
    async def run(self, **kwargs) -> dict[str, Any]:
        ...
        targets_data = kwargs.get('targets_data')

        # ✅ FIX: Explicitly retrieve enriched data from kwargs or load from Parquet
        if enriched_data is None:
            self.logger.info("Data not in pipeline_data, attempting to load from Parquet...")
            try:
                enriched_data = pd.read_parquet("data/processed/features/features.parquet")
                targets_data = pd.read_parquet("data/processed/features/targets.parquet")
            except Exception as e:
                self.logger.error(f"❌ Failed to load data from Parquet: {e}")
                return {"status": "failed", "reason": "data_missing"}

        # ✅ BUG FIX: Safe alignment-aware merge of targets into enriched_data.
        # Previously this was a direct column assignment by integer index which silently
        # introduces NaN rows when enriched_data and targets_data have different lengths
        # or different indices (e.g. after concat / reset_index in Stage 3).
        if isinstance(enriched_data, pd.DataFrame) and targets_data is not None and isinstance(targets_data, pd.DataFrame):
            target_cols = [c for c in targets_data.columns if c.startswith('target_')]
            if target_cols:
                # Prefer merging on datetime+ticker for semantic alignment
                merge_keys = [k for k in ('datetime', 'ticker', 'interval') if k in enriched_data.columns and k in targets_data.columns]
                if merge_keys:
                    targets_subset = targets_data[merge_keys + target_cols].drop_duplicates(subset=merge_keys)
                    enriched_data = enriched_data.merge(targets_subset, on=merge_keys, how='left', suffixes=('', '_target_dup'))
                    # Drop any duplicate columns that appeared due to suffix
                    dup_cols = [c for c in enriched_data.columns if c.endswith('_target_dup')]
                    if dup_cols:
                        enriched_data.drop(columns=dup_cols, inplace=True)
                    nan_rows = enriched_data[target_cols[0]].isna().sum() if target_cols[0] in enriched_data.columns else 0
                    self.logger.info(f"✅ Merged {len(target_cols)} target columns via {merge_keys} ({nan_rows} unmatched rows).")
                else:
                    # Fallback: direct assignment only when lengths match exactly
                    if len(enriched_data) == len(targets_data):
                        for t_col in target_cols:
                            enriched_data[t_col] = targets_data[t_col].values
                        self.logger.info(f"✅ Assigned {len(target_cols)} target columns (lengths matched).")
                    else:
                        self.logger.warning(
                            f"⚠️ Cannot merge targets: no common key columns and lengths differ "
                            f"({len(enriched_data)} vs {len(targets_data)}). Targets skipped."
                        )
    ...
    async def _process_successful_training(self, config: SuccessfulTrainingConfig):
        ...
        # Persist light model results to disk so Stage 5/6 can pick them up
        if light_models_trained:
            self._save_light_models_results(
                ticker=config.ticker,
                target_name=config.target_name,
                light_models=light_models_trained,
                batch_dir=batch_dir
            )

        # Log to experience diary
        self._log_to_diary(champion_info, config.timeframe)
        for _light_key, light_info in light_models_trained.items():
            self._log_to_diary(light_info, config.timeframe)

    def _save_light_models_results(
        self,
        ticker: str,
        target_name: str,
        ...
        """
        Persists light model training results to light_models_results.json.
        Accumulates results across multiple tickers/targets without overwriting.
        """
        results_file = batch_dir / "light_models_results.json"
        try:
            # Load existing results (if any real model data exists)
            existing: dict[str, Any] = {}
            if results_file.exists():
                with open(results_file, encoding="utf-8") as f:
                    raw = json.load(f)
                # Only keep if it has real ticker data (not just metadata keys)
                if any(k not in ("batch_name", "created", "last_updated", "runs") for k in raw):
                    existing = raw

            # Merge new results under ticker -> target -> model_type
            ...
            for _context_key, info in light_models.items():
                ...
                target_data[model_type] = {
                    ...
                    "accuracy": info.get("metrics", {}).get("accuracy",
                                info.get("metrics", {}).get("score", 0.0)),
            ...
            logger.info(
                f"✅ Saved light model results for {ticker}/{target_name} "
                f"({len(target_data)} models) -> {results_file.name}"
            )
        except Exception as e:
        ...
        for model_type in light_model_types:
            try:
                training_config = SingleModelTrainingConfig(
                    ...
                    ticker=ticker,
                    target_name=target_name,
                    timeframe=self._resolve_timeframe(target_name),
                    batch_dir=batch_dir,
                    x_train=x_train,
                    y_train=y_train,
                    x_test=x_test,
    ...
    def _determine_task_type(self, target_name: str) -> str:
        """Determine if this is a regression, classification, or multi-class task.

        ✅ CALC FIX: Added explicit multi-class detection ('multi' in name).
        Previously 'target_return_multi_1d' was misclassified as regression.
        """
        name_lower = target_name.lower()
        if 'multi' in name_lower or 'class' in name_lower:
            return 'multiclass'
        if 'return' in name_lower or 'price' in name_lower or 'value' in name_lower:
            return 'regression'
        return 'classification'

    def _resolve_timeframe(self, target_name: str) -> str:
        """Infer timeframe from target name, fallback to '1d'."""
        for tf in ('15m', '60m', '1h', '1d', '4h', '1w'):
            if tf in target_name:
                return tf
        return '1d'
