...
class LightModelsTrainer:
    ...
    async def run_light_models(self, features_df: pd.DataFrame, targets_df: pd.DataFrame,
    ...
    async def _save_light_model_results(self, results: dict[str, Any], timestamp: str) -> dict[str, Any]:
        ...
        accumulated_results = self._load_or_create_accumulated_results(light_results_path, current_run)

        async with aiofiles.open(light_results_path, 'w', encoding='utf-8') as f:
            import json
            await f.write(json.dumps(accumulated_results, indent=2, default=str))
    ...
    def _load_or_create_accumulated_results(self, results_path: Path, current_run: dict[str, Any]) -> dict[str, Any]:
        ...
        if results_path.exists():
            try:
                ...
                safe_path = self._validate_file_path(results_path)
        ...
        return {
            ...
            'created': current_run['timestamp'],
            'last_updated': current_run['timestamp'],
            'runs': [current_run]
        }

    async def run_light_models_with_selected_features(self, features_df: pd.DataFrame, targets_df: pd.DataFrame,
                                                     config: LightModelTrainingConfig) -> dict[str, Any]:
        """Run light models with pre-selected features."""
        self.logger.info(f"Training light models with pre-selected features for {len(config.light_models_to_train)} models...")

        # Load selected features
        selected_features = self._load_selected_features(config.selected_features_files, config.test_model)
        if not selected_features:
            self.logger.warning("No selected features found, using all features")
            selected_features = [col for col in features_df.columns if not col.startswith('target_')]

        # Filter features
        filtered_features = features_df[selected_features + ['ticker', 'timeframe', 'datetime'] + config.target_cols]

        # Train models
        return await self.run_light_models(filtered_features, targets_df)

    def _load_selected_features(self, selected_features_files: list[str], model_name: str) -> list[str]:
        ...
        for features_file in selected_features_files:
            ...
            if features_path.exists():
                try:
                    # Validate file path to prevent path traversal
                    safe_path = self._validate_file_path(features_path)
                    with open(safe_path, encoding='utf-8') as f:
    ...
    def _validate_file_path(self, file_path: Path) -> Path:
        ...
        try:
            from src.utils.path_utils import validate_path_within_directory
            return validate_path_within_directory(file_path, Path.cwd())
        except ImportError:
            # Fallback validation if path_utils not available
