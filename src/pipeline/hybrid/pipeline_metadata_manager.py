# src/pipeline/hybrid/pipeline_metadata_manager.py
"""
...
class PipelineMetadataManager:
    ...
    def __init__(self, output_dir: Path, batch_name: str, light_models: List[str], heavy_models: List[str]):
    ...
    def _create_pipeline_metadata(self, timestamp: str, tickers: Optional[List[str]], 
                                  timeframes: Optional[List[str]], stages: List[int], 
    ...
    def _load_or_create_accumulated_results(self, light_results_path: Path, current_run: Dict[str, Any]) -> Dict[str, Any]:
        """Load existing or create new accumulated results."""
        accumulated_results = {'timestamp': datetime.now().isoformat(), 'total_runs': 1, 'runs': [current_run]}
        
        if light_results_path.exists():
            try:
                with open(light_results_path, 'r', encoding='utf-8') as f:
                    existing = json.load(f)
                    accumulated_results['total_runs'] = existing.get('total_runs', 0) + 1
                    accumulated_results['runs'] = existing.get('runs', []) + [current_run]
                    self.logger.info(f"📊 Accumulated {accumulated_results['total_runs']} runs")
    ...
    async def _save_light_model_results(self, results: Dict[str, Any], timestamp: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"✅ Light models results accumulated: {light_results_path}")
        return {
