...
class ResultsProcessor:
    ...
    def load_colab_results(self, batch_name: str, output_dir: Path) -> dict[str, Any]:
        """Loads training results from Colab."""
        batch_dir = self._find_batch_directory(batch_name, output_dir)
        if not batch_dir:
            self.logger.warning(f"⚠️ Batch directory not found for {batch_name}")
        ...
        # Ensure batch name is present for downstream logic
        if isinstance(results, dict) and 'batch_name' not in results:
            try:
                results['batch_name'] = batch_dir.name
            except Exception:
                results['batch_name'] = batch_name

        # Basic validation: ensure models_metadata or ticker_results exists
        if not isinstance(results, dict) or (not results.get('models_metadata') and not results.get('ticker_results')):
            self.logger.warning(f"⚠️ Loaded Colab results appear incomplete: {results_path}")

        self.logger.info(f"✅ Loaded Colab results from {results_path}")
        return results

    def _find_batch_directory(self, batch_name: str, output_dir: Path) -> Path | None:
        ...
        # Log for debugging
        self.logger.debug(f"🔍 Searching for batch directory: batch_name={batch_name}, eff_batch_name={eff_batch_name}, output_dir={output_dir}")

        # Try exact match with output_dir
        ...
        # If output_dir is the parent of accumulated (e.g. data/colab/accumulated)
        # we might need to go one level up and search
        search_root = output_dir
        if output_dir.name == batch_name or output_dir.name == eff_batch_name:
            search_root = output_dir.parent

        # Try to find similar batch directories in the search root
        try:
            self.logger.debug(f"🔍 Searching for similar directories in {search_root}")
            similar = [d for d in search_root.iterdir()
                      if d.is_dir() and (eff_batch_name in d.name or d.name in eff_batch_name)]
            if similar:
    ...
    def _find_results_file(self, batch_dir: Path) -> Path | None:
        ...
        files_to_try = [
            batch_dir / "colab_results_summary.json",
            batch_dir / "colab_results.json",
    ...
    def _convert_single_model_path(self, model_path: str, batch_dir: Path) -> str:
    ...
    def build_models_metadata(self, colab_results: dict[str, Any],
        ...
        # Add colab results metadata
        if 'models_metadata' in colab_results:
            models_metadata.update(colab_results['models_metadata'])
            self._update_selected_features_from_ticker_results(models_metadata, colab_results)

        # Add light results metadata
    ...
    def _update_selected_features_from_ticker_results(self, models_metadata: dict[str, Any],
                                                      colab_results: dict[str, Any]) -> None:
        ...
        for ticker, ticker_data in ticker_results.items():
            for _timeframe, timeframe_data in ticker_data.get('timeframes', {}).items():
                for target, target_data in timeframe_data.get('results', {}).items():
                    for model, model_data in target_data.get('models', {}).items():
                        self._update_model_selected_features(models_metadata, ticker, target, model, model_data)

    def _update_model_selected_features(self, models_metadata: dict[str, Any], ticker: str,
                                       target: str, model: str, model_data: dict[str, Any]) -> None:
        """Update selected features for a specific model."""
        key = f"{ticker}_{target}_{model}"
        if key in models_metadata:
            models_metadata[key]['selected_features'] = model_data.get('selected_features', [])
