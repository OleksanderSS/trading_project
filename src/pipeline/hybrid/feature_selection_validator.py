...
class FeatureSelectionValidator:
    ...
    def check_if_feature_selection_needed(self, batch_dir: Path, new_rows_count: int,
    ...
    def _check_initial_or_forced_selection(self, batch_dir: Path, force: bool) -> dict[str, Any] | None:
    ...
    def _check_time_based_selection(self, batch_dir: Path) -> dict[str, Any] | None:
        ...
        try:
            ...
            file_path = Path(files[0]).resolve()
    ...
    def create_mock_selected_features_for_test(self, request: MockFeaturesRequest) -> list[Path]:
        """Create mock selected_features files for testing when Colab results are not available."""
        self.logger.info(f"Creating mock selected_features files: {request.test_ticker}, {request.test_target}")

        valid_features = self._extract_valid_features(request.features_df)
        mock_selected_features = valid_features[:20] if len(valid_features) >= 20 else valid_features
        ...
        mock_files_request = MockFeatureFilesRequest(
            ...
            light_models=request.light_models,
    ...
    def _write_mock_feature_files(self, request: MockFeatureFilesRequest) -> list[Path]:
        ...
        for model_name in request.light_models:
            ...
            safe_ticker = ''.join(c for c in request.test_ticker if c.isalnum() or c in ['_', '-'])
            safe_target = ''.join(c for c in request.test_target if c.isalnum() or c in ['_', '-'])
            file_name = f"selected_features_{safe_model}_{safe_ticker}_{safe_target}.json"
            file_path = request.batch_dir / file_name
            # Ensure the file is within batch_dir
            if not str(file_path.resolve()).startswith(str(request.batch_dir.resolve())):
                raise ValueError("Path traversal detected")
