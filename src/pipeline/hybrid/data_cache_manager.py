...
class DataCacheManager:
    ...
    def handle_data_caching(self, local_res: dict[str, Any], force_training: bool,
                           batch_name: str, output_dir: Path) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    ...
    def _has_new_data(self, features_path: Path, new_features: pd.DataFrame) -> bool:
        ...
        try:
            ...
            # Create row identity sets using datetime plus available instrument/timeframe columns.
            known = self._row_identity_set(old_features)
            current = self._row_identity_set(new_features)
            if not current:
                self.logger.warning("Could not build row identities for new data; treating cache as stale")
                return True
    ...
    def _row_identity_set(self, df: pd.DataFrame) -> set:
        """Build stable row identities for freshness checks without losing timeframe context."""
        if 'datetime' not in df.columns:
            return set()

        normalized = pd.to_datetime(df['datetime'], errors='coerce').dt.tz_localize(None)
        keys = [normalized]

        for col in ('ticker', 'timeframe', 'interval'):
            if col in df.columns:
                keys.append(df[col].astype(str))

        return set(zip(*keys, strict=False))
