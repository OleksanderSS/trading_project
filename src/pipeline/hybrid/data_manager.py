# src/pipeline/hybrid/data_manager.py
"""
...
class HybridDataManager:
    ...
    def __init__(self, output_dir: Path):
        # output_dir already includes batch_name!
        # e.g., data/colab/accumulated/main_database
        self.output_dir = output_dir
        self.logger = ProjectLogger.get_logger(self.__class__.__name__)

    def load_features_data(self) -> pd.DataFrame:
        """Load features data from parquet file."""
        # output_dir is already the batch directory
        ...
        try:
            ...
            self.logger.error(f"Error loading features from {features_path}: {e}")
            return pd.DataFrame()
    ...
    def load_targets_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error(f"Error loading targets from {targets_path}: {e}")
            return pd.DataFrame()

    def save_dataframes(self, features_df: pd.DataFrame, targets_df: pd.DataFrame) -> dict[str, Any]:
    ...
    def has_new_data(self, features_path: Path, new_features: pd.DataFrame) -> bool:
        """Check if new data exists compared to cache."""
        try:
            old_features = pd.read_parquet(features_path)
            known = set(zip(pd.to_datetime(old_features['datetime']).dt.tz_localize(None), old_features['ticker'], strict=False))
            current = set(zip(pd.to_datetime(new_features['datetime']).dt.tz_localize(None), new_features['ticker'], strict=False))
