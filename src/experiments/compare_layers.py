...
class CompareLayersExperiment(BaseExperiment):
    ...
    def run_single_test(self, test_params: dict) -> dict:
        ...
        try:
            ...
            ensemble_result, _ = ensemble_forecast(
    ...
    def run_experiment(self, **kwargs) -> List[dict]:
        ...
        test_cases = self._generate_test_cases(start_date, end_date)
    ...
    def _generate_test_cases(self, start_date: datetime, end_date: datetime) -> List[dict]:
    ...
    def _execute_test_cases(self, test_cases: List[dict]) -> List[dict]:
        """Execute test cases in parallel or sequential mode"""
        if self.exp_config.get('parallel', True):
            return self._run_parallel(test_cases, self.exp_config.get('workers', 4))
        else:
            return [self.run_single_test(case) for case in test_cases]
