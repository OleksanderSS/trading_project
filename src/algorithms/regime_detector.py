...
class MarketRegimeDetector:
    ...
    def _detect_regime_ml(self, returns: np.ndarray, prices: np.ndarray | None,
        ...
        try:
            ...
            # ✅ Dynamically calibrate K-Means cluster centers to adapt to multi-year non-stationarity
            alpha = self.config.get('centroid_learning_rate', 0.01)
            if hasattr(model, 'cluster_centers_') and model.cluster_centers_ is not None:
                model.cluster_centers_[cluster] = (1.0 - alpha) * model.cluster_centers_[cluster] + alpha * features_scaled[0]
                self.logger.debug(f"[REGIME] Calibrated centroid for cluster {cluster} (regime {regime.value}) with alpha {alpha}")
    ...
    def _extract_ml_features(self, returns: np.ndarray, prices: np.ndarray | None,
        ...
        """Витягує features для ML кластеризації з фіксованою довжиною (11)"""
        features = []

        # Remove NaNs from returns
        valid_returns = returns[~np.isnan(returns)]
        if len(valid_returns) < 5:
            # Fallback if too many NaNs
            valid_returns = np.zeros(5)

        # Return-based features (6)
        features.extend([
            float(np.mean(valid_returns)),
            float(np.std(valid_returns)),
            float(stats.skew(valid_returns) if len(valid_returns) > 2 else 0),
            float(stats.kurtosis(valid_returns) if len(valid_returns) > 3 else 0),
            float(np.min(valid_returns)),
            float(np.max(valid_returns))
        ])
        ...
        if prices is not None:
            ...
            if len(valid_prices) > 20:
                features.extend(self._extract_price_features(valid_prices))
            else:
                features.extend([0.0, 50.0, 0.0])  # trend, RSI, bb_position fallbacks
        else:
            features.extend([0.0, 50.0, 0.0])

        # Volume-based features (1)
        if volume is not None:
            valid_volume = volume[~np.isnan(volume)]
            if len(valid_volume) > 20:
                features.append(self._extract_volume_feature(valid_volume))
            else:
                features.append(1.0)
        else:
            features.append(1.0)
        ...
        if sentiment is not None:
            valid_sentiment = sentiment[~np.isnan(sentiment)]
            if len(valid_sentiment) > 0:
                features.append(float(np.mean(valid_sentiment)))
            else:
                features.append(0.0)
        else:
            features.append(0.0)

        # Ensure there are no NaNs in the final features
    ...
    def _normalize_features(self, features: list[float]) -> np.ndarray:
        ...
        # Use transform instead of fit_transform so that we don't scale a single sample to all 0s!
    ...
    def _initialize_cluster_centers(self):
        ...
        # Fit standard scaler once on predefined centers
        self.scaler.fit(centers)
        centers_scaled = self.scaler.transform(centers)

        seed = self.config_manager.get('performance.random_seed', 42)
        self.cluster_model = KMeans(n_clusters=self.n_clusters, init=centers_scaled, n_init=1, random_state=seed)
        # Fit the model on the predefined scaled centers
        self.cluster_model.fit(centers_scaled)
