...
class RiskParityAllocator:
    ...
    def _get_allocation_methods(self, vols: np.ndarray, correlations: np.ndarray | None, assets: list[str]):
        ...
        return {
            ...
            AllocationMethod.EQUAL_WEIGHT: self._create_equal_weight_method(n_assets),
            AllocationMethod.RISK_PARITY: self._risk_parity
        }

    def _create_equal_weight_method(self, n_assets: int):
        """Creates a bound equal weight method for specific asset count"""
        def _equal_weight_method(_v: np.ndarray, _c, constraints: dict[str, Any]) -> np.ndarray:
            return self._equal_weight(n_assets, constraints)
        return _equal_weight_method
    ...
    def _hierarchical_risk_parity(self, vols: np.ndarray, correlations: np.ndarray,
        ...
        try:
            ...
            clustering = AgglomerativeClustering(
                ...
                metric=self.hrp_distance_metric if self.hrp_distance_metric != 'precomputed' else 'euclidean'
            )

            clustering.fit(distance_matrix)

            # 1. Створюємо квазі-діагональний порядок (Quasi-Diagonalization) за Lopez de Prado
            def get_quasi_diagonal(children: np.ndarray) -> list[int]:
                def get_leaves(node_idx: int) -> list[int]:
                    if node_idx < n_assets:
                        return [node_idx]
                    left, right = children[node_idx - n_assets]
                    return get_leaves(left) + get_leaves(right)

                root_idx = 2 * n_assets - 2
                return get_leaves(root_idx)

            ordered_assets = get_quasi_diagonal(clustering.children_)

            # 2. Рекурсивно розподіляємо ваги по кластерах за допомогою бісекції
            ordered_weights = self._hrp_recursive_allocation(ordered_assets, vols, correlations)

            # 3. Картуємо ваги назад до оригінального порядку активів
            weights = [0.0] * n_assets
            for asset_idx, w in zip(ordered_assets, ordered_weights, strict=False):
                weights[asset_idx] = w
    ...
    def _hrp_recursive_allocation(self, cluster_items: list[int],
        ...
        # Розбиваємо навпіл для бісекції
        ...
        left_weights = self._hrp_recursive_allocation(left_cluster, vols, correlations)
        right_weights = self._hrp_recursive_allocation(right_cluster, vols, correlations)

        # Комбінуємо та масштабуємо ваги
        final_weights = [0.0] * len(cluster_items)
        for idx, val in enumerate(left_weights):
            final_weights[idx] = left_weight * val
        for idx, val in enumerate(right_weights):
            final_weights[mid + idx] = right_weight * val
