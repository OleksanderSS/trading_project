...
class SmartModelSelector:
    ...
    def _get_historical_reliability(self, model_name: str, target_type: str, context: Dict) -> float:
        ...
        for key in self.performance_history:
            if key.startswith(model_name) and key.endswith(target_type):
                recent_success = np.mean([r['metrics'].get('accuracy', 0.5) for r in self.performance_history[key]["runs"][-5:]])
    ...
    def _get_historical_score(self, model_name: str, target_type: str) -> Optional[float]:
