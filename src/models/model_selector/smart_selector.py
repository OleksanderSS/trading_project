...
class SmartModelSelector:
    ...
    def __init__(self, results_file: str = "model_performance_history.json", competence_map_file: str = "src/models/model_selector/model_competence_map.json"):
        ...
        # Мета-модель для Predictionування помилок (ядро Критика)
        self.error_meta_model = RandomForestRegressor(
            n_estimators=50, max_depth=5, min_samples_leaf=2, max_features='sqrt', random_state=42
        )
    ...
    def _load_competence_map(self, file_path: str) -> dict[str, Any]:
        try:
    ...
    def train_error_meta_model(self, model_name: str, ticker: str):
        ...
        if history_key not in self.performance_history or not self.performance_history[history_key].get("runs"):
            logger.warning(f"Недостатньо історії для Training мета-моделі для {history_key}")
            return
    ...
    def critique_action(self, action: dict[str, Any], context_df: pd.DataFrame) -> dict[str, Any]:
        ...
        historical_reliability = self._get_historical_reliability(model_name, ticker, target_type)

        critique_score = self._calculate_critique_score(expected_error, context_adjustment, historical_reliability)
        points = self._generate_critique_points(context, expected_error, context_adjustment, historical_reliability)
    ...
    def _get_expected_error(self, context: dict) -> float:
        ...
        try:
            ...
            return float(self.error_meta_model.predict(context_for_pred)[0])
        except Exception:
            return 0.5

    def _get_historical_reliability(self, model_name: str, target_type: str, context: dict) -> float:
        """Get historical reliability for model"""
        for key in self.performance_history:
            if key.startswith(model_name) and key.endswith(target_type):
                recent_success = float(np.mean([r['metrics'].get('accuracy', 0.5) for r in self.performance_history[key]["runs"][-5:]]))
                return recent_success
        return 0.5

    def _calculate_critique_score(self, expected_error: float, context_adjustment: float, historical_reliability: float) -> float:
    ...
    def _generate_critique_points(self, context: dict, expected_error: float, context_adjustment: float, historical_reliability: float) -> list[str]:
    ...
    def calculate_model_score(self, model_name: str, target_type: str, context: dict) -> float:
    ...
    def _get_historical_score(self, model_name: str, target_type: str) -> float | None:
        """Calculate historical performance score for model"""
        for key in self.performance_history:
            if key.startswith(model_name) and key.endswith(target_type):
                scores = self._calculate_run_scores(self.performance_history[key].get("runs", []))
                if scores:
    ...
    def _calculate_run_scores(self, runs: list[dict]) -> list[float]:
        ...
        for run in runs:
            ...
            weighted_score, total_weight = self._calculate_weighted_score(metrics)
            if total_weight > 0:
    ...
    def _get_default_model_score(self, model_name: str, target_type: str) -> float:
        return float(self.competence_map.get("default_scores", {}).get(target_type, {}).get(model_name, 0.5))
    ...
    def select_best_model(self, df: pd.DataFrame, target_type: str,
        ...
        model_scores = {name: self.calculate_model_score(name, target_type, context) for name in available_models}
