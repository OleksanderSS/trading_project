...
class EnhancedEnsembleModel:
    ...
    def __init__(self, models_path: str = "data/models"):
        ...
        self.heavy_models = {}
        self.light_models = {}
        self.diary = None  # DiaryEngine placeholder
        self.logger = logger
    ...
    def _process_models_metadata(self, models_metadata: Dict, batch_name: str) -> Dict[str, Any]:
        """Process models metadata format"""
        heavy_models = {}
        
        for model_key, model_data in models_metadata.items():
            if not isinstance(model_data, dict) or model_data.get('type') != 'heavy':
                continue
            ...
            logger.debug(f"✅ Added heavy model: {model_key} (MSE: {model_data.get('mse', 0):.6f})")
    ...
    def _process_timeframes_data(self, timeframes: Dict, ticker: str) -> Dict[str, Any]:
        ...
        for timeframe, tf_data in timeframes.items():
            ...
            for target_name, target_result in results.items():
                if not isinstance(target_result, dict) or 'models' not in target_result:
                    continue
                    
                models = target_result.get('models', {})
                for model_type, model_data in models.items():
                    if not isinstance(model_data, dict) or 'mse' not in model_data:
                        continue
                        
                    model_key = f"{ticker}_{target_name}_{model_type}"
                    model_path = model_data.get('model_path', '')
                    model_obj = self._load_model_from_path(model_path, 'main_database')
    ...
    def _load_model_from_path(self, model_path: str, batch_name: str) -> Optional[Any]:
        """Load model from various possible paths"""
        if not model_path:
            return None
        ...
        for path_candidate in possible_paths:
            if Path(path_candidate).exists():
                try:
                    ...
                    logger.debug(f"⚠️ Failed to load model {path_candidate}: {e}")
    ...
    def _load_single_model(self, model_file: Path, light_model_types: List[str]) -> Optional[Dict[str, Any]]:
        """Load a single model file and return model info"""
        try:
            model = joblib.load(model_file)
            stem = model_file.stem
            
            model_info = self._parse_model_filename(stem)
            if not model_info or model_info['model_type'] not in light_model_types:
                return None
    ...
    def load_local_light_models(self, target_cols: List[str] = None, 
                            tickers: List[str] = None, 
                            timeframes: List[str] = None) -> Dict[str, Any]:
        """Load ALL 8 types of light models locally considering timeframe"""
        logger.info("📥 Loading ALL 8 types of light models locally (with TF support)...")
        
        light_models = {}
        light_model_types = ['catboost', 'lightgbm', 'xgboost', 'random_forest', 
                         'linear', 'svm', 'knn', 'ensemble']
        ...
        for model_file in model_files:
            ...
            if self._should_skip_model(model_info, target_cols, tickers, timeframes):
                continue
            
            model_key = f"{model_data['ticker']}_{model_data['target']}_{model_data['model_type']}"
    ...
    def _parse_model_filename(self, stem: str) -> Optional[Dict[str, str]]:
        ...
        if stem.startswith('CHAMP_'):
            # Format: CHAMP_{ticker}_{timeframe}_{target}
            parts = stem[6:].split('_')  # Remove 'CHAMP_'
            if len(parts) >= 3:
                ticker = parts[0]
                timeframe = parts[1]
            ...
            # Format: {ticker}_{timeframe}_{target}_{model_type}
            parts = stem.split('_')
            if len(parts) >= 4:
                ticker = parts[0]
                timeframe = parts[1]
                target = parts[2]
                model_type = "_".join(parts[3:])  # Handle model types with underscores
    ...
    def get_model_statistics(self) -> Dict[str, Any]:
        ...
        return {
            'heavy_models_count': len(self.heavy_models),
            'light_models_count': len(self.light_models),
            'total_models': len(self.heavy_models) + len(self.light_models),
            'heavy_models': list(self.heavy_models.keys())[:10],
            'light_models': list(self.light_models.keys())[:10],
        }
    ...
    def vectorized_comparison(
        ...
        for ticker in self._get_unique_tickers(features_df):
            ...
            for target_col in target_cols:
                comparison_key = f"{ticker}_{target_col}"
                comparison_results[comparison_key] = self._create_comparison_result(
                    ticker, target_col, ticker_features
                )
    ...
    def _get_ticker_features(self, features_df: pd.DataFrame, ticker: str) -> pd.DataFrame:
        """Get features for specific ticker"""
        return features_df[features_df['ticker'] == ticker]
    
    def _create_comparison_result(self, ticker: str, target_col: str, 
                            ticker_features: pd.DataFrame) -> Dict[str, Any]:
        """Create comparison result for ticker-target combination"""
        if target_col not in ticker_features.columns:
            return None
            
        target_data = ticker_features[target_col].dropna()
        ...
        return {
            'ticker': ticker,
            'target': target_col,
            'heavy_quality': 0.5,  # Placeholder
            'heavy_predictions': [],
            'light_predictions': [],
            'light_qualities': []
        }
    
    def create_ensemble(self, comparison_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _create_single_ensemble(self, comp_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not light_predictions and not heavy_predictions and heavy_quality == 0:
            return None
            
        # Simple ensemble without KNN correction for now
        if heavy_predictions and light_predictions:
            ensemble_pred = np.mean(heavy_predictions + light_predictions, axis=0)
        ...
        return {
            ...
            'light_quality': np.mean(light_qualities) if light_qualities else 0,
            'heavy_predictions': heavy_predictions,
            'light_predictions': light_predictions,
            'n_heavy': len(heavy_predictions),
            'n_light': len(light_predictions),
            'knn_applied': False,
            'n_models': len(heavy_predictions) + len(light_predictions),
        }
