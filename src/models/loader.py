...
class ModelLoaderStrategy:
    ...
    def load_model(self, model_meta: Dict[str, Any]) -> Optional[Any]:
        ...
        # Try loader strategies if direct path loading is not possible
    ...
    def load_path(self, model_path: str, meta: Dict[str, Any]) -> Optional[Any]:
        ...
        try:
            ...
            raise ModelLoadingError(f"Failed to load model from path {model_path}: {e}") from e
        
        raise ModelLoadingError(f"Unsupported model file suffix: {path.suffix}")
    
    def _load_local_model(self, model_path: str, meta: Dict[str, Any]) -> Optional[Any]:
        """
    ...
    def _load_colab_model(self, model_path: str, meta: Dict[str, Any]) -> Optional[Any]:
        """
    ...
    def _load_consensus_model(self, model_path: str, meta: Dict[str, Any]) -> Optional[Any]:
        """
        Fallback: Load consensus meta-model.
        
        Uses a pre-trained consensus model as fallback when specific model fails to load.
        This provides graceful degradation - predictions still work even if model is missing.
        
        Args:
            model_path: Original model path (unused for fallback)
            meta: Model metadata
        
        Returns:
            Loaded consensus model, or None if not found
        """
        ...
        try:
            ...
            self.logger.warning(f"Failed to load consensus model: {e}")
            return None
    
    def _load_stacked_ensemble(self, model_path: str, meta: Dict[str, Any]) -> Optional[Any]:
        """
        Final fallback: Create default stacked ensemble.
        """
        try:
            self.logger.info("Creating default stacked ensemble as final fallback")
            from src.ensembling.stacked_ensemble import StackedEnsemble
            return StackedEnsemble()
        except Exception as e:
            self.logger.error(f"Failed to create default ensemble: {e}")
            return None
    ...
    def _load_joblib(self, path: Path) -> Any:
        ...
        try:
            ...
            raise ModelLoadingError(f"Failed to load joblib model at {path}: {e}") from e

    def _load_pickle(self, path: Path) -> Any:
        self.logger.debug(f"Loading pickle model from {path}")
        try:
            # Joblib is much better at handling numpy/sklearn pickles
            return joblib.load(str(path))
            ...
            try:
                ...
                raise ModelLoadingError(f"Failed to load pickle model at {path} via both joblib ({e1}) and standard pickle ({e2})") from e2
    ...
    def _wrap_keras_model(self, model, model_type: str):
        class KerasPredictor:
            ...
            def predict(self, X):
                ...
                # Reshape for 3D expected input in RNNs/CNNs if necessary
    ...
    def _load_torch_model(self, path: Path, meta: Dict[str, Any]) -> Optional[Any]:
        ...
        try:
            ...
            if isinstance(loaded_obj, dict):
                self.logger.debug(f"Loaded dict from {path}, resolving state_dict or wrapper")
                state_dict = None
                scaler = None
                ...
                pytorch_model = self._create_pytorch_model(model_type, input_size)
                pytorch_model.load_state_dict(state_dict)
                return self._wrap_pytorch_model(pytorch_model, model_type, scaler)
            else:
                return self._wrap_pytorch_model(loaded_obj, meta.get('model_type', path.stem))
        except Exception as e:
            raise ModelLoadingError(f"Failed to load PyTorch model from {path}: {e}") from e

    def _extract_input_size(self, state_dict: Dict[str, Any]) -> int:
        self.logger.debug("Extracting input size from state_dict")
        for key, val in state_dict.items():
            if 'weight' in key and hasattr(val, 'shape') and len(val.shape) >= 2:
                return int(val.shape[1])
        self.logger.warning("Unable to determine input size from state_dict, using fallback 47")
        return 47
...
class ModelLoaderFactory:
    ...
    @staticmethod
    def create_loader(logger: Optional[logging.Logger] = None) -> ModelLoaderStrategy:
        """
    ...
    @staticmethod
    def create_colab_loader(logger: Optional[logging.Logger] = None) -> ModelLoaderStrategy:
        """
        ...
        """
        loader = ModelLoaderStrategy(logger)
        # Reorder loaders to try Colab first
        loader.loaders = [
            loader._load_colab_model,
            loader._load_local_model,
            loader._load_consensus_model,
            loader._load_stacked_ensemble,
        ]
        return loader
