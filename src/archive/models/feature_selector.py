...
class ModelFeatureSelector:
    ...
    def select_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        cols_to_use = [c for c in df.columns if not is_target_like_column(c)]
        ...
        available_features = [f for f in available_features if not is_target_like_column(f)]

        # Обмежуємо до 42 фіч
        selected_features = available_features[: self.max_features]

        # Final validation
        final_selected = [f for f in selected_features if not is_target_like_column(f)]

        # Leakage guard: ensure no column in final_selected starts with 'target_'
        if any(is_target_like_column(f) for f in final_selected):
    ...
    def select_additional_features(self, df: pd.DataFrame, exclude_features: list[str]) -> list[str]:
        ...
        numeric_features = [
            ...
            and not is_target_like_column(col)
        ]
