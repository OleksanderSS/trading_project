...
class AdvancedBacktester:
    ...
    def _realized_vol(self, price_series: pd.Series | None, end_idx: int, window: int = 20) -> float:
        """Causal realized volatility fallback when volatility_data is absent.

        Computes the std of recent log returns up to (not including) end_idx,
        so it never peeks at the bar being traded. Falls back to a conservative
        2% only when there is not enough history (first bars).
        """
        if price_series is None or end_idx <= 1:
            return 0.02
        start = max(0, end_idx - window)
        slice_ = price_series.iloc[start:end_idx].dropna()
        if len(slice_) < 2:
            return 0.02
        rets = np.log(slice_ / slice_.shift(1)).dropna()
        if rets.empty:
            return 0.02
        std = float(rets.std())
    ...
    def run_backtest(self,
                     ...
                     signal_data: pd.Series | pd.DataFrame,
                     initial_capital: float | None = None,
                     volume_data: pd.Series | pd.DataFrame | None = None,
                     volatility_data: pd.Series | pd.DataFrame | None = None,
                     **kwargs) -> dict[str, Any]:
        """
        ...
        for i in range(1, len(combined_data)):
            ...
            for ticker in tickers:
                ...
                if abs(trade_value) > self.minimum_trade_value:
                    price = combined_data.iloc[i][f'{ticker}_price']
                    # Realized volatility fallback: derive from prices when no
                    # explicit volatility_data is supplied, instead of a flat
                    # 2% constant that distorts slippage/impact estimates.
                    vol = self._realized_vol(price_df.get(ticker), i)
