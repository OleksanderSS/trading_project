...
class OnlineModelIntegration:
    ...
    def _determine_market_phase(self, data_row: pd.Series) -> str:
        ...
        try:
            ...
            logger.warning(f"Error determining market phase: {e}")
            return "unknown"
    ...
    def _get_model_prediction(self, model_data: Any, X: np.ndarray) -> float | None:
        ...
        try:
            ...
            if strategy == 'best_only':
                ...
                if directions:
                    ...
                    same_direction_preds = [pred for pred, direction in zip(predictions.values(), directions, strict=True)
