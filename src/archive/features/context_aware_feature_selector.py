...
class ContextAwareFeatureSelector:
    ...
    def select_features(
        ...
        # Adaptive top_k based on volatility (VIX)
        adaptive_top_k = self.top_k
        vix_col = next((col for col in X_clean.columns if 'vix' in col.lower()), None)
        if vix_col is not None and not X_clean[vix_col].empty:
            recent_vix = X_clean[vix_col].iloc[-20:].mean()
            if recent_vix > 30:
                # High volatility: cut features by 20% to reduce noise/overfitting
                adaptive_top_k = max(5, int(self.top_k * 0.8))
                logger.info(f"High volatility (VIX={recent_vix:.2f}). Reducing top_k from {self.top_k} to {adaptive_top_k}")
            elif recent_vix > 20:
                # Moderate volatility: cut features by 10%
                adaptive_top_k = max(5, int(self.top_k * 0.9))
                logger.info(f"Moderate volatility (VIX={recent_vix:.2f}). Reducing top_k from {self.top_k} to {adaptive_top_k}")

        if len(X_clean) < 10:
            logger.warning(f"Insufficient data after cleaning: {len(X_clean)} rows")
            return feature_names[:adaptive_top_k], {}

        # Update selector k if needed
        if hasattr(self.selector, 'k') and self.selector.k != adaptive_top_k:
            self.selector.k = adaptive_top_k

        # Perform selection
        if self.method == 'random_forest':
            selected_features, importances = self._select_with_random_forest(
                X_clean, y_clean, feature_names, adaptive_top_k
            )
        else:
        ...
        logger.info(f"✅ Selected {len(selected_features)} features (Target: {adaptive_top_k}):")
    ...
    def _select_with_sklearn(
        ...
        try:
            ...
            # Fallback: return first adaptive_top_k features
            return feature_names[:adaptive_top_k], {}
    ...
    def _select_with_random_forest(
        ...
        try:
            ...
            # Sort by importance and select adaptive_top_k
            sorted_features = sorted(
                importances.items(),
                key=lambda x: x[1],
                reverse=True
            )
            selected_features = [name for name, _ in sorted_features[:adaptive_top_k]]
