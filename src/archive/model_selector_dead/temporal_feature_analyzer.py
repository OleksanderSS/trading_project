...
class TemporalFeatureAnalyzer:
    ...
    def analyze_temporal_patterns(self, df: pd.DataFrame, ticker: str,
        ...
        weekly_returns = df.groupby('week')['close'].pct_change(fill_method=None).mean()
        ...
        monthly_returns = df.groupby('month')['close'].pct_change(fill_method=None).mean()
    ...
    def _analyze_feature_temporal_patterns(self, df: pd.DataFrame, feature: str) -> dict[str, Any]:
        ...
        patterns['daily'] = {
            ...
            'weekend_effect': df[df['day_of_week'].isin([5, 6])][feature].mean() / (df[df['day_of_week'].isin([0, 1, 2, 3, 4])][feature].mean() + 1e-9)
        }
