...
class StackedEnsemble:
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, use_oof_validation: bool = True):
        """
        Trains the meta-model on base model predictions.
        
        Args:
            X: Base model predictions (should be OOF predictions to prevent leakage)
            y: True target values
            use_oof_validation: If True, validates that predictions are OOF to prevent data leakage
        """
        self.feature_names = X.columns.tolist()

        # Validate OOF predictions to prevent data leakage
        if use_oof_validation:
            for col in X.columns:
                correlation = X[col].corr(y)
                if correlation > 0.95:
                    logger.warning(
                        f"[StackedEnsemble] Model '{col}' has very high correlation ({correlation:.3f}) "
                        f"with target. This may indicate data leakage. Ensure OOF predictions are used."
                    )
