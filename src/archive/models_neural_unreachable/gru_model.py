...
class GRUModel(BaseNeuralModel):
    ...
    def train(self, X: np.ndarray, y: np.ndarray, **kwargs) -> dict[str, Any]:
        ...
        Args:
            X: Input data (2D or 3D). If 2D, will be converted to real
               rolling-window sequences via sequence_builder (not a fake
               reshape treating each feature column as a timestep).
            y: Target data
            **kwargs: Additional arguments including window_size, step_size
    ...
    def predict(self, X: np.ndarray, **kwargs) -> np.ndarray:
        ...
        Args:
            X: Input data (2D or 3D). If 2D, will be converted to real
               rolling-window sequences via sequence_builder.
            **kwargs: Additional arguments including window_size, step_size
        """
