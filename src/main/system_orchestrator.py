...
class SystemOrchestrator:
    ...
    async def run_mode(self, mode: str, tickers: (list[str] | None)=None,
    ...
    async def _execute_mode(self, config: ExecutionConfig, **kwargs) ->dict[
        str, Any]:
        ...
        if config.mode in ['train', 'predict', 'backtest']:
            ...
            return await self._run_hybrid_mode(config.tickers, config.
                timeframes, **kwargs)
            ...
            return await self._run_intelligent_mode(config.tickers, config.
                timeframes, config.parallel, **kwargs)
        elif config.mode == 'monster_test':
            return await self._run_monster_test(config.tickers, config.
                timeframes, config.parallel, **kwargs)
        else:
    ...
    async def _run_training_data_pipeline(self) ->dict[str, Any]:
        ...
        return {'status': 'success', 'message':
            'Training data pipeline completed successfully.'}
    ...
    def _handle_mode_error(self, mode: str, error: Exception) ->dict[str, Any]:
        ...
        self.logger.critical(
            f"A critical error occurred while executing mode '{mode}': {error}"
            , exc_info=True)
        return {'status': 'critical_failure', 'error': str(error)}

    async def _dispatch(self, mode_class: Any, config: ExecutionConfig, **
        kwargs) ->dict[str, Any]:
        """
        ...
        if self._should_run_parallel(config.tickers, config.parallel):
            return self._run_parallel_execution(mode_class, config, results
                =results, **kwargs)
        else:
            return await self._run_sequential_execution(mode_class, config,
                results=results, **kwargs)

    def _should_run_parallel(self, tickers: (list[str] | None), parallel: bool
        ) ->bool:
        """Check if execution should run in parallel."""
        return tickers is not None and parallel and len(tickers) > 1

    def _run_parallel_execution(self, mode_class: Any, config:
        ExecutionConfig, results: dict[str, Any], **kwargs) ->dict[str, Any]:
    ...
    async def _run_sequential_execution(self, mode_class: Any, config:
        ExecutionConfig, results: dict[str, Any], **kwargs) ->dict[str, Any]:
        """Run mode execution sequentially."""
        result = await self._run_single_instance(mode_class, config.tickers,
            config.timeframes, **kwargs)
    ...
    async def _run_hybrid_mode(self, tickers: (list[str] | None),
        ...
        orchestrator = HybridOrchestrator(self.config_manager, batch_name=
            batch_name)
        request = HybridPipelineRequest(tickers=tickers, timeframes=
            timeframes, accumulate=kwargs.pop('accumulate', True),
            force_training=kwargs.pop('force_training', False), skip_colab=
            kwargs.pop('skip_colab', False), force_feature_selection=kwargs
            .pop('force_feature_selection', False))
    ...
    async def _run_intelligent_mode(self, tickers: (list[str] | None),
        ...
        try:
            if hasattr(dean_brain, 'experience_diary'
                ) and dean_brain.experience_diary.needs_retraining(tickers or
                []):
                mode_type = TrainMode
                self.logger.info(
            ...
            raise
        config = ExecutionConfig(mode='intelligent', tickers=tickers,
            timeframes=timeframes, parallel=parallel)
        return await self._dispatch(mode_type, config, brain=dean_brain, **
            kwargs)

    async def _run_monster_test(self, tickers: (list[str] | None),
        timeframes: (list[str] | None), parallel: bool, **kwargs) ->dict[
        str, Any]:
        """
