...
class WebUIMode(BaseMode):
    ...
    def __init__(self):
        super().__init__()
        # Fixed imports - use existing modules
        try:
            from src.monitoring import ModelPerformanceMonitor
            self.performance_monitor = ModelPerformanceMonitor()
        except ImportError:
            self.performance_monitor = None
            self.logger.warning("ModelPerformanceMonitor not available, using None")
        
        # RealDataCollector doesn't exist, create placeholder
    ...
    def _log_startup_info(self, host: str, port: int) -> None:
        ...
        self.logger.info("💼 Trading Interface: http://%s:%d/trading", host, port)
        self.logger.info("[UP] System Overview: http://%s:%d/dashboard", host, port)
    ...
    def _handle_startup_error(self, error: Exception, host: str, port: int) -> Dict[str, Any]:
    ...
    def _create_trading_ui_handler(self):
        ...
        class TradingUIHandler(http.server.SimpleHTTPRequestHandler):
            ...
            def _get_api_response(self):
                ...
                api_handlers = {
                    '/api/system/overview': self.web_ui_mode.get_system_overview,
                    '/api/portfolio/status': self.web_ui_mode.get_portfolio_status,
                    '/api/market/data': self.web_ui_mode.get_market_data,
                    '/api/performance/metrics': self.web_ui_mode.get_performance_metrics
                }
            ...
            def get_html_content(self, filename):
                html_handlers = {
                    'index.html': self.web_ui_mode.get_index_html,
                    'dashboard.html': self.web_ui_mode.get_dashboard_html,
                    'trading.html': self.web_ui_mode.get_trading_html
                }
    ...
    def get_system_overview(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'config': {
                ...
                'timeframes': self.config_manager.get('data.timeframes', []),
                'initial_capital': self.config_manager.get('risk.initial_capital', 0)
            },
    ...
    def get_market_data(self) -> Dict[str, Any]:
        market_data = {}
        tickers = self.config_manager.get('data.tickers', ['TSLA', 'NVDA', 'AAPL', 'MSFT', 'GOOGL'])
        import random
        ...
        for ticker in tickers[:5]:
            try:
                ...
                self.logger.warning("Failed to get market data for %s: %s", ticker, str(e))
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            'recent_activity': [
                {'time': datetime.now().strftime('%H:%M'), 'action': 'BUY', 'ticker': 'TSLA', 'quantity': 10, 'price': 245.50},
                {'time': datetime.now().strftime('%H:%M'), 'action': 'SELL', 'ticker': 'AAPL', 'quantity': 5, 'price': 178.25},
                {'time': datetime.now().strftime('%H:%M'), 'action': 'BUY', 'ticker': 'NVDA', 'quantity': 8, 'price': 485.75}
            ],
            'total_trades': 156,
            'win_rate': 0.65,
            'avg_return': 0.023,
    ...
    def _get_performance_report(self) -> Dict[str, Any]:
        ...
        if self.performance_monitor:
            try:
                ...
                self.logger.warning("Failed to get performance report: %s", str(e))
