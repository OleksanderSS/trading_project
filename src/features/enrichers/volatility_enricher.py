    ...
    def _enrich_impl(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            if 'close' in df_enriched.columns:
                ...
                df_enriched['volatility_5'] = df_enriched['returns'].rolling(window=5).std() * np.sqrt(252)
                df_enriched['volatility_10'] = df_enriched['returns'].rolling(window=10).std() * np.sqrt(252)
                ...
                if all(col in df_enriched.columns for col in ['high', 'low', 'close']):
                    ...
                    df_enriched['atr_14'] = true_range.rolling(window=14).mean()

                # Garman-Klass Volatility
                if 'high' in df_enriched.columns and 'low' in df_enriched.columns:
                    gk = (0.5 * (np.log(df_enriched['high']) - np.log(df_enriched['low']))**2 -
                          (2 * np.log(2) - 1) * (np.log(df_enriched['close']) - np.log(df_enriched['close'].shift()))**2)
                    df_enriched['gk_volatility'] = gk.rolling(window=20).sum()
