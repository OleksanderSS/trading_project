...
class AdvancedAnalyticsEnricher(BaseEnricher):
    ...
    def _add_macro_composite_score(self, df_enriched: pd.DataFrame) -> None:
        ...
        fred_cols = [col for col in df_enriched.columns if col and isinstance(col, str) and col.startswith('FRED_')]
    ...
    def _add_market_phase_detection(self, df_enriched: pd.DataFrame) -> None:
        ...
        try:
            ...
            # Map market_regime to numeric if it exists (for MarketPhaseAnalyzer rules)
            temp_df = df_enriched.copy()
            if 'market_regime' in temp_df.columns and temp_df['market_regime'].dtype == object:
                regime_map = {
                    'TRENDING_UP': 0,
                    'TRENDING_DOWN': 1,
                    'RANGING': 2,
                ...
                temp_df['market_regime'] = temp_df['market_regime'].map(regime_map).fillna(8)

            # Analyze phase for each row (or just use last row for all)
            # For efficiency, we'll detect phase once and apply to all rows
            phase_result = self.phase_analyzer.analyze({'market_data': temp_df})
