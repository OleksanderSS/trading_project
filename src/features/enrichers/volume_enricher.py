    ...
    def _enrich_impl(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if "volume" in df_enriched.columns and "close" in df_enriched.columns:
            if "ticker" in df_enriched.columns:
                groups = [
                    self._compute_volume_indicators(group.copy())
                    for _ticker, group in df_enriched.groupby("ticker")
                ]
                df_enriched = pd.concat(groups).sort_index()
            else:
                df_enriched = self._compute_volume_indicators(df_enriched)

            logger.info(f"✅ Added {6} volume indicators")

        return df_enriched

    def _compute_volume_indicators(self, df_enriched: pd.DataFrame) -> pd.DataFrame:
        """Compute volume indicators for a single ticker's rows (chronological)."""
