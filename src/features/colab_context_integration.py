...
class ContextAwareColabFeatureSelector:
    ...
    def _init_selector(self):
        ...
        try:
            ...
            # Fallback to SmartFeatureSelector
            try:
            ...
            # Final fallback: SimpleFeatureSelector
    ...
    def select_features(
        ...
        if len(ticker_features) < 10:
            raise ValueError(f"Insufficient data for {ticker}: {len(ticker_features)} rows")
        ...
        # Perform selection
        if self.uses_context_aware and hasattr(self.feature_selector, 'select_features'):
            ...
            selected_names = feature_names[:min(max_features, len(feature_names))]
            analysis = {
                'base_count': len(selected_names),
                'context_count': 0,
                'temporal_count': 0,
                'uses_context': False
            }
        
        # Convert to indices and array
        ...
        print(f"   Base: {analysis.get('base_count', 0)}, "
              f"Context: {analysis.get('context_count', 0)}, "
              f"Temporal: {analysis.get('temporal_count', 0)}")
...
def visualize_context_importance(
    analysis: Dict[str, Any],
