...
class FeatureSelectionCache:
        ...
        if self.cache_file.exists():
            try:
                with open(self.cache_file, 'r') as f:
                    cache = json.load(f)
                logger.info(f"✅ Loaded feature selection cache: {len(cache)} entries")
                return cache
    ...
    def _save_cache(self):
        ...
        try:
            ...
            logger.debug(f"💾 Saved feature selection cache: {len(self.cache)} entries")
        except Exception as e:
    ...
    def _compute_cache_key(
        ...
        """
        # Сортуємо фічі для консистентності
        ...
        cache_key = f"{model_type}_{target_name}_{market_regime}_{features_hash}"
        return cache_key
    ...
    def get_selection(
        ...
        cache_key = self._compute_cache_key(
            model_type, target_name, market_regime, available_features
        )
        
        if cache_key in self.cache:
            cached_data = self.cache[cache_key]
            logger.info(f"🚀 Feature selection cache HIT: {cache_key}")
            logger.info(f"   Selected {len(cached_data['selected_features'])} features")
            return cached_data
        else:
    ...
    def save_selection(
        ...
        cached_data = {
            ...
            'selection_ratio': len(selected_features) / len(available_features) if available_features else 0
        }
        ...
        logger.info(f"   Selected {len(selected_features)}/{len(available_features)} features")
    ...
    def invalidate(
        ...
        """
        if model_type is None and target_name is None and market_regime is None:
            # Інвалідувати весь кеш
            self.cache = {}
            self._save_cache()
            logger.info("🗑️ Invalidated entire feature selection cache")
            return
        
        # Інвалідувати частково
        keys_to_remove = []
        for cache_key, cached_data in self.cache.items():
            ...
            if target_name and cached_data.get('target_name') != target_name:
                match = False
            if market_regime and cached_data.get('market_regime') != market_regime:
                match = False
...
def get_feature_selection_cache(cache_dir: Optional[str] = None) -> FeatureSelectionCache:
    """Отримати глобальний інстанс кешу"""
    global _global_cache
    
    if _global_cache is None:
        _global_cache = FeatureSelectionCache(
            cache_dir=cache_dir or 'data/cache/feature_selection'
        )
