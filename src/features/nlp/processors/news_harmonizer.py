...
def detect_news_format(entry: dict) -> dict:
    ...
    logger.debug(f"[news_harmonizer] Using fields: date={date_field}, text={text_field}")
    return {
...
def harmonize_entry(entry: dict, source: str, default_type: str = "qualitative") -> dict:
    ...
    if pd.isna(pub_dt):
        logger.debug("[news_harmonizer] [DEBUG] Invalid date, using current time")
...
def harmonize_batch(entries: list, source: str, default_type: str = "qualitative") -> list:
    ...
    for entry in entries:
        ...
        if item["description"]:
            ...
            logger.warning("[news_harmonizer] [WARN] Skipping entry without description")
    logger.info(f"[news_harmonizer] [OK] Harmonized {len(harmonized)} of {len(entries)} entries")
    return harmonized
