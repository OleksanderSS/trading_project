...
class FileManager:
    ...
    def save_yaml(self, data: Dict[str, Any], file_path: Union[str, Path], async_save: bool = False) -> None:
        ...
        if async_save:
            self._executor.submit(self._atomic_write, path, write_task, validate_task)
        else:
    ...
    def save_dataframe(
        ...
        **kwargs
        ...
        df_to_save = self._remove_timezone(df.copy()) if remove_tz else df.copy()
        ...
        def write_task(p: Path):
            if format == 'parquet':
                ...
                df_to_save.to_json(p, orient='records', date_format='iso', **kwargs)
            else:
        ...
        def validate_task(p: Path) -> bool:
            try:
                if format == 'parquet':
                    pd.read_parquet(p, columns=[df_to_save.columns[0]]) # Partial load check
                elif format == 'csv':
    ...
    def cleanup_old_files(self, directory: Union[str, Path], max_age_days: int = 7, pattern: str = "*") -> None:
        ...
        if not path.exists() or not path.is_dir():
            self.logger.warning(f"Cleanup skipped: {path} is not a valid directory.")
            return
