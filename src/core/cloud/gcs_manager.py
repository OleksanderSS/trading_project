...
class GCSManager:
    ...
    def upload_file(self, source_file_path: str, destination_blob_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to upload file '{source_file_path}' to '{destination_blob_name}': {e}", exc_info=True)
            return False
    ...
    def upload_blob_from_memory(self, file_obj, destination_blob_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to upload from memory to '{destination_blob_name}': {e}", exc_info=True)
            return False

    def download_file(self, source_blob_name: str, destination_file_path: str) -> bool:
        """Downloads a file from the GCS bucket."""
        try:
            logger.info(f"Downloading GCS path '{source_blob_name}' to '{destination_file_path}'...")
            ...
            logger.error(f"Failed to download file '{source_blob_name}' to '{destination_file_path}': {e}", exc_info=True)
            return False
    ...
    def list_files(self, prefix: Optional[str] = None) -> list[str]:
        ...
        try:
            ...
            logger.error(f"Failed to list files with prefix '{prefix}': {e}", exc_info=True)
            return []
    ...
    def file_exists(self, blob_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking for existence of blob '{blob_name}': {e}", exc_info=True)
            return False
    ...
    def wait_for_blob(self, blob_name: str, timeout: int = 300) -> Optional[storage.Blob]:
        ...
        while time.time() - start_time < timeout:
            ...
            logger.info(f"Waiting for blob '{blob_name}'... polling again in 10 seconds.")
            time.sleep(10)
        logger.warning(f"Timed out waiting for blob '{blob_name}' after {timeout} seconds.")
        return None
