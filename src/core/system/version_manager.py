"""
System Version Management - Управління версіями системи, конфігурацій та моделей
"""
...
class ConfigVersionManager:
    """
    Менеджер версій системи з підтримкою моделей та прогонів пайплайну
    """
    
    def __init__(self, history_path: Optional[str] = None):
        self.history_path = Path(history_path or "src/config/version_history.json")
        self.versions: List[ConfigVersion] = []
        self.current_version = "1.3.0"
        self._load_versions()
    
    def _load_versions(self):
        """Завантажує історію версій з JSON файлу"""
        if not self.history_path.exists():
            logger.warning(f"Version history file not found at {self.history_path}. Initializing with defaults.")
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.versions)} versions from {self.history_path}")
        except Exception as e:
            logger.error(f"Failed to load version history: {e}")
            self.versions = self._get_default_versions()

    def _save_versions(self):
        """Зберігає історію версій у JSON файл"""
        try:
    ...
    def _get_default_versions(self) -> List[ConfigVersion]:
        """Початкові версії, якщо файл відсутній"""
        return [
            ConfigVersion(
                version="1.0.0", date="2024-01-01", description="Initial version",
                changes=["Basic collector configuration", "Thresholds system"]
            ),
            ConfigVersion(
                version="1.3.0", date="2024-04-08", description="Unified system",
                changes=["Unified configuration", "Enhanced error handling"],
                migration_required=True, migration_script="migrate_to_v1_3_0"
            )
        ]

    def get_current_version(self) -> str:
        """Отримання поточної версії системи"""
        return self.current_version

    def get_model_version(self, ticker: str, model_type: str) -> str:
        """
        Генерує версію для моделі на основі дати та типу.
        Формат: {model_type}_{ticker}_{YYYYMMDD_HHMM}
        """
    ...
    def tag_pipeline_run(self) -> str:
        """Генерує унікальний ID для поточного прогону пайплайну"""
    ...
    def get_migration_path(self, from_version: str, to_version: Optional[str] = None) -> List[str]:
        """Отримання шляху міграції"""
        target_version = to_version or self.current_version
        from_idx = next((i for i, v in enumerate(self.versions) if v.version == from_version), None)
        to_idx = next((i for i, v in enumerate(self.versions) if v.version == target_version), None)
        
        if from_idx is None or to_idx is None or from_idx >= to_idx:
            return []
        
        return [self.versions[i].version for i in range(from_idx + 1, to_idx + 1)]
    
    def needs_migration(self, from_version: str) -> bool:
        """Перевірка чи потрібна міграція"""
...
# Глобальний екземпляр менеджера версій
_version_manager = None


def get_version_manager() -> ConfigVersionManager:
    """Отримання глобального екземпляра менеджера версій"""
