...
class IErrorHandler(ABC):
    ...
    @abstractmethod
    def handle_error(self, error: Exception, context: str = "General", severity: str = "error", should_raise: bool = False) -> Dict[str, Any]:
...
def safe_execute(func: Callable, *args, **kwargs):
    ...
    try:
        ...
        # Re-raise known trading system errors without modification
        raise
    except Exception as e:
        logger.error(f"Unexpected error in {func.__name__}: {e}", exc_info=True)
        # Wrap unexpected errors in a generic TradingSystemError
        raise TradingSystemError(f"An unexpected error occurred in {func.__name__}: {e}") from e


# --- ErrorHandler Class for Stateful Error Management ---
...
class ErrorHandler(IErrorHandler):
    ...
    def _format_error_message(self, error_info: Dict[str, Any]) -> str:
        ...
        message = (
            f"<b>🚨 {error_info.get('severity', 'ERROR').upper()} 🚨</b>\n"
            f"<b>Time:</b> {error_info.get('timestamp')}\n"
            f"<b>Type:</b> {error_info.get('error_type')}\n"
            f"<b>Context:</b> <code>{context_str}</code>\n"
            f"<b>Message:</b> <pre>{error_info.get('error_message')}</pre>\n"
        )
        return message
    ...
    def handle_error(
        ...
        if severity == "critical":
            self.logger.critical(f"Critical failure in {context}: {error_message}", exc_info=True)
        elif severity == "error":
            self.logger.error(log_message, exc_info=True)
        elif severity == "warning":
            self.logger.warning(log_message)

        # Increment error count for this type of error
        error_key = f"{str(context)}:{error_type}"
        self.error_counts[error_key] = self.error_counts.get(error_key, 0) + 1
        self.error_history.append(error_info)

        # Handle Notifications with Cooldown
        if severity in ["critical", "error"] and self.notifier:
            now = datetime.now()
            last_notified = self.notification_cooldowns.get(error_key)
            
            if not last_notified or (now - last_notified) > timedelta(minutes=self.cooldown_minutes):
                notification_message = self._format_error_message(error_info)
                self.notifier.sync_send(notification_message, level=severity.upper())
    ...
    def retry(self, max_retries: int = 3, delay: float = 1.0, backoff: float = 2.0, exceptions: tuple = (Exception,)):
        """
    ...
    def log_performance(self, func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration = datetime.now() - start_time
                self.logger.error(f"{func.__name__} failed after {duration}: {str(e)}")
                raise
        return wrapper
...
def get_error_handler(config_manager: Optional[UnifiedConfigManager] = None) -> ErrorHandler:
...
class ErrorContext:
    ...
    def __init__(
        self,
        operation: str,
        error_type: type = TradingSystemError,
        log_level: str = "ERROR"
    ):
