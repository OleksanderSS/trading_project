...
### synthetic_gates / EXCEPTION_FALLS_BACK_TO_SAMPLE_DATA — `data/collectors/put_call_ratio_collector.py:162`
...
### error_policy / BROAD_EXCEPTION_SILENT_RETURN — `features/selection/smart_selector.py:261`
**Problem:** Broad exception returns silent fallback: None.
**Why:** Pipeline may continue with None/{}/[] as if the stage succeeded.
**Fix:** Return a typed StageResult with status failed/degraded, or re-raise for fatal stages.
**Test:** Test that fatal failures in target generation/split/training do not continue silently.
...
### error_policy / BROAD_EXCEPTION_SILENT_RETURN — `features/selection/smart_selector.py:306`
**Problem:** Broad exception returns silent fallback: None.
**Why:** Pipeline may continue with None/{}/[] as if the stage succeeded.
**Fix:** Return a typed StageResult with status failed/degraded, or re-raise for fatal stages.
**Test:** Test that fatal failures in target generation/split/training do not continue silently.
...
### error_policy / BROAD_EXCEPTION_SILENT_RETURN — `features/selection/smart_selector.py:317`
**Problem:** Broad exception returns silent fallback: None.
**Why:** Pipeline may continue with None/{}/[] as if the stage succeeded.
**Fix:** Return a typed StageResult with status failed/degraded, or re-raise for fatal stages.
**Test:** Test that fatal failures in target generation/split/training do not continue silently.
...
### missing_policy / FILLNA_ZERO_SUSPICIOUS — `features/enrichers/sentiment_features_enricher.py:328`
**Problem:** fillna(0) detected.
**Why:** Zero-filling targets, returns, risk metrics, or financial features can fabricate labels/returns and suppress risk.
**Fix:** Replace with explicit per-column policy: drop targets, leave unavailable features as NaN plus availability indicator, or domain-specific fill.
**Test:** Add tests that target tails and missing price gaps are not converted to zeros.
...
### missing_policy / FILLNA_ZERO_SUSPICIOUS — `features/enrichers/sentiment_features_enricher.py:342`
**Problem:** fillna(0) detected.
**Why:** Zero-filling targets, returns, risk metrics, or financial features can fabricate labels/returns and suppress risk.
**Fix:** Replace with explicit per-column policy: drop targets, leave unavailable features as NaN plus availability indicator, or domain-specific fill.
**Test:** Add tests that target tails and missing price gaps are not converted to zeros.
...
**Confidence:** high  
```python
returns = ticker_df['close'].pct_change().fillna(0).dropna().values
```

### missing_policy / PCT_CHANGE_IMPLICIT_FILL_METHOD — `pipeline/stages/trading/recommendation_engine.py:169`
**Problem:** pct_change() called without explicit fill_method.
**Why:** Depending on pandas version/defaults, missing values may be forward-filled before returns are computed.
**Fix:** Use pct_change(fill_method=None), then explicitly handle NaN/inf according to data kind.
**Test:** Add a test where a missing price gap does not become a zero or forward-filled return.
...
### missing_policy / PCT_CHANGE_IMPLICIT_FILL_METHOD — `pipeline/stages/trading/recommendation_engine.py:201`
**Problem:** pct_change() called without explicit fill_method.
**Why:** Depending on pandas version/defaults, missing values may be forward-filled before returns are computed.
**Fix:** Use pct_change(fill_method=None), then explicitly handle NaN/inf according to data kind.
**Test:** Add a test where a missing price gap does not become a zero or forward-filled return.
...
**Confidence:** high  
```python
symbol_returns = close_df[symbol].pct_change().dropna()
```
...
### security / PLACEHOLDER_SECRET_REVIEW — `features/feature_orchestrator.py:163`
**Problem:** Placeholder-looking secret/default detected.
**Why:** Placeholders can be mistaken for valid credentials or leak into production config.
**Fix:** Validate secrets at startup and reject known placeholder patterns.
**Test:** Test that placeholder secrets fail validation.
...
### temporal / NEGATIVE_SHIFT_LOOKAHEAD — `pipeline/guards/temporal_target_guard.py:33`
**Problem:** Negative shift detected. It may be valid for target generation, but dangerous elsewhere.
**Why:** shift(-h) reads future rows. Without ticker grouping and tail-row dropping, it creates lookahead/cross-ticker leakage.
**Fix:** Use it only in target modules, group by ticker, and drop/mark the last horizon rows instead of filling them.
**Test:** Create a multi-ticker fixture and assert last row of each ticker has NaN target and never uses the next ticker.
...
### temporal / NEGATIVE_SHIFT_LOOKAHEAD — `pipeline/guards/temporal_target_guard.py:34`
**Problem:** Negative shift detected. It may be valid for target generation, but dangerous elsewhere.
**Why:** shift(-h) reads future rows. Without ticker grouping and tail-row dropping, it creates lookahead/cross-ticker leakage.
**Fix:** Use it only in target modules, group by ticker, and drop/mark the last horizon rows instead of filling them.
**Test:** Create a multi-ticker fixture and assert last row of each ticker has NaN target and never uses the next ticker.
...
### config_factory / HARDCODED_MODEL_LIST — `monitoring/example_usage.py:192`
**Problem:** Hardcoded model list detected.
**Why:** Duplicated model lists across factory/arena/pipeline drift over time.
**Fix:** Move models/aliases/capabilities into one registry/config and reference it everywhere.
**Test:** Test that factory, CLI, arena, and prediction stage resolve the same registry entries.
...
### error_policy / LOGGER_ERROR_THEN_RAISE — `data/collectors/put_call_ratio_collector.py:162`
**Problem:** Exception is logged and re-raised in the same handler.
**Why:** If upper layers also log, this creates duplicate error reports and noisy traces.
**Fix:** Log only at boundary layers, or add context and re-raise without error-level logging.
**Test:** Add a test/logger capture for one error event per failing operation.
...
### splits / RANDOM_TRAIN_TEST_SPLIT — `calibration/calibration_engine.py:208`
...
### splits / RANDOM_TRAIN_TEST_SPLIT — `training/portfolio_optimizer.py:202`
**Problem:** train_test_split(shuffle=False) detected.
**Why:** While chronological, this split lacks a 'purge gap' >= target horizon, which may lead to leakage if observations overlap.
**Fix:** Use chronological or purged time split with gap >= max target horizon.
**Test:** Assert train max timestamp < validation min timestamp and purge gap >= target horizon.
...
### splits / RANDOM_TRAIN_TEST_SPLIT — `training/portfolio_optimizer.py:240`
**Problem:** train_test_split(shuffle=False) detected.
**Why:** While chronological, this split lacks a 'purge gap' >= target horizon, which may lead to leakage if observations overlap.
**Fix:** Use chronological or purged time split with gap >= max target horizon.
**Test:** Assert train max timestamp < validation min timestamp and purge gap >= target horizon.
...
### data_lineage / FEATURE_WITHOUT_LOCAL_LINEAGE_HINT — `features/selection/smart_selector.py:237`
**Problem:** Feature/enricher code updates data without nearby lineage/availability metadata.
**Why:** Trading features need source, ticker/timeframe granularity, calculation window, and availability time.
**Fix:** Add feature manifest entries or emit lineage metadata from each enricher.
**Test:** Test that every emitted feature has source, window, granularity, availability_time, causal flag.
...
### data_lineage / FEATURE_WITHOUT_LOCAL_LINEAGE_HINT — `features/selection/smart_selector.py:243`
**Problem:** Feature/enricher code updates data without nearby lineage/availability metadata.
**Why:** Trading features need source, ticker/timeframe granularity, calculation window, and availability time.
**Fix:** Add feature manifest entries or emit lineage metadata from each enricher.
**Test:** Test that every emitted feature has source, window, granularity, availability_time, causal flag.
...
### data_lineage / FEATURE_WITHOUT_LOCAL_LINEAGE_HINT — `features/selection/smart_selector.py:260`
**Problem:** Feature/enricher code updates data without nearby lineage/availability metadata.
**Why:** Trading features need source, ticker/timeframe granularity, calculation window, and availability time.
**Fix:** Add feature manifest entries or emit lineage metadata from each enricher.
**Test:** Test that every emitted feature has source, window, granularity, availability_time, causal flag.
...
### data_lineage / FEATURE_WITHOUT_LOCAL_LINEAGE_HINT — `features/selection/smart_selector.py:305`
**Problem:** Feature/enricher code updates data without nearby lineage/availability metadata.
**Why:** Trading features need source, ticker/timeframe granularity, calculation window, and availability time.
**Fix:** Add feature manifest entries or emit lineage metadata from each enricher.
**Test:** Test that every emitted feature has source, window, granularity, availability_time, causal flag.
...
### data_lineage / FEATURE_WITHOUT_LOCAL_LINEAGE_HINT — `features/selection/smart_selector.py:313`
**Problem:** Feature/enricher code updates data without nearby lineage/availability metadata.
**Why:** Trading features need source, ticker/timeframe granularity, calculation window, and availability time.
**Fix:** Add feature manifest entries or emit lineage metadata from each enricher.
**Test:** Test that every emitted feature has source, window, granularity, availability_time, causal flag.
...
### determinism / NON_INJECTED_CLOCK — `data/collectors/put_call_ratio_collector.py:112`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `data/collectors/put_call_ratio_collector.py:151`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `data/collectors/put_call_ratio_collector.py:157`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `data/collectors/put_call_ratio_collector.py:173`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `monitoring/example_usage.py:237`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `monitoring/example_usage.py:286`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `monitoring/example_usage.py:293`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `pipeline/guards/temporal_target_guard.py:138`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `pipeline/hybrid/model_training_orchestrator.py:65`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `pipeline/hybrid/model_training_orchestrator.py:112`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `pipeline/hybrid/model_training_orchestrator.py:151`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `pipeline/hybrid/model_training_orchestrator.py:157`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `pipeline/hybrid/model_training_orchestrator.py:173`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
...
### determinism / NON_INJECTED_CLOCK — `pipeline/stages/trading/recommendation_engine.py:491`
**Problem:** Direct current-time call detected.
**Why:** Runtime and tests become nondeterministic; relative dates can drift.
**Fix:** Inject a clock/reference_now parameter or central time provider.
**Test:** Freeze clock in tests and assert stable outputs.
