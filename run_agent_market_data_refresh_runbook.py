...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--collector-inventory-json",
        default=LIVE_COLLECTOR_INVENTORY,
        help=(
            "Path to a saved collector-inventory JSON, or 'live' (default) to scan "
            "src/config/collectors.yaml now. Saved snapshots are reported as "
            "'snapshot_*' status because nothing regenerates them any more."
        ),
    )
...
def print_summary(payload: dict) -> None:
    ...
    print(f"Collector inventory: {summary.get('collector_inventory_status')}")
