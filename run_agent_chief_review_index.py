...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run ChiefReviewIndexBuilder (chief_review_index).")
    parser.add_argument("--no-save", dest="save", action="store_false", help="Build the payload without writing report files.")
    parser.add_argument("--output-dir", default="reports/dean_os/chief_review_index_current")
...
def main() -> None:
    ...
    payload = ChiefReviewIndexBuilder(output_dir=args.output_dir).build(
        save=args.save,
    )
    if args.print_json:
        print_json(payload)
