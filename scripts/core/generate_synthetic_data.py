...
class SyntheticDataGenerator:
    ...
    def run(self, scenario_types: Optional[List[str]] = None):
        """
        ...
        try:
            ...
            for i in range(n_simulations):
                ...
                scenario = {
                    ...
                    'type': 'monte_carlo',
                    'is_synthetic': True,  # ← Маркування
                    'synthetic_type': 'typical',  # ← Тип синтетичних даних
        ...
        try:
            ...
            for shock_name, shock_params in shock_types.items():
                ...
                scenario = {
                    ...
                    'type': 'shock',
                    'is_synthetic': True,  # ← Маркування
                    'synthetic_type': 'shock',  # ← Тип синтетичних даних
    ...
    def _generate_context_scenarios(self):
        ...
        try:
            ...
            for regime_name, regime_params in regimes.items():
                ...
                # Ensure regime_params is treated as a dictionary
                if not isinstance(regime_params, dict):
                    continue
                    
                regime_dict = cast(dict, regime_params)
                trend = float(regime_dict.get('trend', 0))
                volatility = float(regime_dict.get('volatility', 0.1))
                ...
                scenario = {
                    ...
                    'type': 'context',
                    'is_synthetic': True,  # ← Маркування
                    'synthetic_type': 'context',  # ← Тип синтетичних даних
                    'regime': regime_name,
    ...
    def _simulate_dean_bootstrap(self, regime_name: str) -> Dict[str, Any]:
        """Simulate DEAN bootstrap for regime using actual DEAN system"""
        
        try:
            # Create scenario for DEAN
            scenario = {
                'regime': regime_name,
                'market_state': self._get_market_state_for_regime(regime_name),
                'horizon': 252,  # 1 year simulation
                'scenario_id': f'context_{regime_name}'
            }
            
            # Use actual DEAN internal simulation
            dean_result = self.dean_system.internal_simulation(scenario)
            
            # Convert DeanSimulation to dict
            return {
                'regime': regime_name,
                'scenario_id': dean_result.scenario_id,
                'initial_conditions': dean_result.initial_conditions,
                'predicted_outcomes': dean_result.predicted_outcomes,
                'confidence_distribution': dean_result.confidence_distribution,
                'simulation_steps': dean_result.simulation_steps,
                'avg_confidence': float(np.mean(dean_result.confidence_distribution)) if dean_result.confidence_distribution else 0.0,
                'evolution_metrics': self.dean_system.get_evolution_summary()
            }
            
        except Exception as e:
            logger.warning(f"⚠️ DEAN simulation failed for {regime_name}, using fallback: {e}")
            # Fallback to simplified version if DEAN fails
            return {
                'regime': regime_name,
                'scenario_id': f'fallback_{regime_name}',
                'predicted_outcomes': [{'state': 'unknown', 'confidence': 0.5}],
                'confidence_distribution': [0.5],
                'simulation_steps': [],
                'error': str(e)
            }
    
    def _get_market_state_for_regime(self, regime_name: str) -> Dict[str, Any]:
        """Convert regime name to market state for DEAN"""
        ...
        return regime_configs.get(regime_name, {
            'trend': 'neutral',
            'volatility': 'moderate',
            'momentum': 'neutral',
            'risk_level': 'moderate'
