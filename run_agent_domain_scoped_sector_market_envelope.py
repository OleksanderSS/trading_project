...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Verify one saved sector-market artifact as a review-only "
    ...
    parser.add_argument(
        ...
        default=(
            "reports/dean_os/"
            "saved_sector_market_evidence_producer_current/latest.json"
        ),
    )
    parser.add_argument(
        "--dispatch-path",
    ...
    parser.add_argument(
        ...
        default=(
            "reports/dean_os/"
            "domain_scoped_sector_market_envelope_current"
        ),
    )
...
def main() -> int:
    ...
    payload = DomainScopedSectorMarketEnvelope(args.output_dir).build(
