...
class PerformanceTracker:
    ...
    def checkpoint(self, name: str):
        ...
        print(f" Checkpoint '{name}': {elapsed:.2f}s, Memory: {stats['memory_mb']:.1f}MB")
...
class ExperimentVisualizer:
    ...
    def plot_metric_comparison(self, df: pd.DataFrame, metrics: List[str], 
        ...
        if save_path:
            ...
            print(f"[DATA] Metric comparison plot saved to {save_path}")
    ...
    def plot_correlation_heatmap(self, df: pd.DataFrame, metrics: List[str],
        ...
        if numeric_df.empty:
            print("[ERROR] No numeric columns for correlation heatmap")
            return
        ...
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"[DATA] Correlation heatmap saved to {save_path}")
    ...
    def plot_best_results(self, df: pd.DataFrame, metric: str, 
        ...
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"[DATA] Best results plot saved to {save_path}")
