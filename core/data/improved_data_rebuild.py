"""
Improved Data Rebuild
Покращена перебудова data with кращою обробкою NaN withначень
"""
...
class ImprovedDataRebuild:
    """Покращена перебудова data"""
    
    def __init__(self):
        self.logger = logging.getLogger("ImprovedDataRebuild")
        
        # Створюємо директорandї
        self.output_dir = Path("data/improved_rebuild")
        ...
        self.logger.info("Improved Data Rebuild initialized")
    ...
    def rebuild_data(self, tickers: List[str] = None, max_tickers: int = 20) -> Dict[str, Any]:
        """Перебудувати данand with покращеною обробкою"""
        if tickers is None:
            tickers = self.priority_tickers[:max_tickers]
        
        self.logger.info(f"[START] Starting improved data rebuild for {len(tickers)} tickers")
        ...
        try:
            ...
            main_path = self.output_dir / f"improved_dataset_{timestamp}.parquet"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Improved Data Rebuild')
    ...
    try:
        ...
        print("\n" + "="*60)
        print("IMPROVED DATA REBUILD RESULTS")
        print("="*60)
