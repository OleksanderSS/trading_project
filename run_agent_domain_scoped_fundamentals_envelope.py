...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Verify one saved terminal SEC fundamentals artifact and its "
            "recursive lineage as a review-only domain binding candidate."
        )
    )
    ...
    parser.add_argument(
        ...
        default=(
            "reports/dean_os/"
            "saved_sec_derived_ratio_producer_current/latest.json"
        ),
    )
    parser.add_argument(
        "--dispatch-path",
    ...
    parser.add_argument(
        ...
        default=(
            "reports/dean_os/"
            "domain_scoped_fundamentals_envelope_current"
        ),
    )
...
def main() -> int:
    ...
    payload = DomainScopedFundamentalsEnvelope(args.output_dir).build(
