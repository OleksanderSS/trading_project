...
### Звірка карти з місцевістю, 04.09 (перенесено з ROADMAP §19a, 06.09 — журнальний запис, не пункт плану)

**Чому це знадобилось.** На ранок 04.09 у карті було **82 невиконані пункти
проти 7 виконаних**. Це вже не план, а накопичення: майже все написане до
Р28-Р36, і «наступний крок», обраний із такого списку, описує проєкт, якого
вже немає. План — теж механізм, і він теж буває мертвим; та сама форма
«оголошено й не діє», від якої цілий день лікували код.

**Закрито виміром: 11 пунктів.** Прогін звіту на всіх ознаках (Р20, Р28),
окремий вимір cftc/wikipedia/insider/fear_greed (Р29, Р31, ворота А),
розгортка горизонту для кожного кандидата (Р23, Р28), витрати як фільтр
(Р22, Р27, Р28), модель витрат (Р22, Р27, Р36), пасивний бенчмарк у кожному
звіті (Р28), дедуплікація (переміряно Р33), умова приймання імені (замінено
Р33 на одну правильнішу), лічильник спроб, печатка, беззбитковість за
горизонтами.

**Підняте в пріоритеті: 2 пункти.**

* **«Середні й малі компанії, не ще двадцять великих»** — Р33 вивела це саме
  незалежно й дала ціну: 3.16 незалежних ставок зі стелі 3.22. **Карта це вже
  знала.** Вимір не відкрив напрямок, він показав, скільки коштує його не
  робити.
* **«Зберігати компанії, що зникають»** — Р34 виміряла ціну відсутності:
  наш власний константний суперник завищений виживанням і дає 20.55% на рік
  крізь крах доткомів.

**Позначено дублікатами: 3 пункти** (`KNNSimilarityFinder` двічі, умовний
вимір IC двічі, «не підбирати умови під результат» — двічі, і це правило, а не
задача).

**Стан після звірки: 18 виконаних, 71 невиконаний.** Решта 71 — це переважно
правильність конвеєра (§1.1, §12, §17, §21, §22, кроки 1-2) і шари, що чекають
на сигнал (агенти, сценарії, політичний шар). **Жоден із них не спростований
сьогоднішніми вимірами — вони просто не про пошук альфи.**

**Чого звірка НЕ зробила.** Не читала кожен із 71 пункту на актуальність по
суті — лише зіставила з Р28-Р36. Пункти, яких сьогоднішні виміри не
торкались, лишені як є, і серед них можуть бути застарілі з інших причин.
...
|---|---|---|---|---|---|
| 300 | закрито | дефект | 07.09, за вимогою власника «зробимо всі пункти, що впливають на правильність» | **[метод]** каталог ролей звітував про 1 390 ознак, а судив **388** — решті 1 002 він або друкував вирок про дані, яких немає, або не лишав рядка взагалі | **ДВІ ФОРМИ, ОБИДВІ НЕВИДИМІ.** **(1)** 935 колонок не лишали в звіті ЖОДНОГО рядка: цикл мав `continue` на колонці без жодного скінченного значення до печатки, тож читач не міг відрізнити «виміряли й нічого не знайшли» від «до виміру не дійшло». **(2)** 67 колонок діставали вирок «market-wide: use as interaction», який видавала одна перевірка `varies < 0.05` — заповнювач її не проходить так само, як справжній макроряд, тож каталог радив **будувати взаємодію зі сталою**. Серед цих 67 — **усі `sentiment_*`, `news_*` і `hype_*` денного кадру: одне значення на 623 398 рядків**. **РОЗРІЗНЮВАЧ — ЧАС:** справжній макроряд не варіює між іменами, але змінюється від дати до дати; заповнювач не змінюється взагалі. Дві величини замість однієї. **200+3 = 203 = 135+67+1** — жодна колонка не загубилась. **ЩО НЕ ЗРУШИЛО, і це перевірено:** `measurable` = **253 в обох прогонах** (колонки без даних несуть NaN p-value і не рахувались як спроби ні до, ні після), заголовок інвентаря **224/4/1166** до і після, вижила та сама `peer_divergence_1d`. **ДЕФЕКТ РІВНЕМ ВИЩЕ, який це викрило:** «виміряно» було визначене як «є в CSV» ще у ДВОХ місцях, і трималось лише випадково — доки звіт пропускав порожні колонки, «відсутнє» і «не згадане» збігались. Щойно 935 почали лишати рядок, обидва почали рахувати їх виміряними; тест впав і сказав це. Тепер визначення одне. **Причину пояснює Р65:** ці колонки сталі не через дефект складальника, а тому, що заповнювати їх нема чим. **Вимір: Р63.** |
| 297 | закрито | гіпотеза | 07.09, з'ясовуючи, чому 15m знову немає в батчі | **[дані]** 15-хвилинний таймфрейм **структурно не здатен дійти до жодного виміру** — не «тихо загубився» | **ВИМІРЯНО НА `features_15m.parquet`: 159 149 рядків, 2026-06-09 .. 2026-08-28 — вісімдесят днів, і ВСІ всередині печатки (початок 2023-09-01). Дослідних рядків НУЛЬ.** І жоден майбутній прогін цього не змінить: Yahoo віддає 15m за шістдесят днів, тож усе вікно, яке ми взагалі здатні отримати, лежить у прихованому періоді **за побудовою**. Отже `timeframes_missing: ['15m']` у метаданих батча — **правильний запис, а не втрата**, і механізм оголошення (той, заради якого писався `test_stage3_timeframe_loss_is_visible`) працює як слід. Та сама сім'я, що Р62: збирач бездоганний, виміру не досягає. **ЩО ЦЕ НЕ ОЗНАЧАЄ:** не привід перестати збирати 15m — паперова торгівля відбувається в теперішньому, де ці бари і є живими. Привід перестати читати їх відсутність у дослідному батчі як дефект. Закріплено `test_fifteen_minute_bars_can_never_reach_a_measurement`, який впаде, якщо колись з'явиться довша внутрішньоденна історія. |
| 296 | закрито | дефект | 07.09, розбираючи 14 червоних юніт-тестів | **[оцінка]** розгорнута оцінка Етапу 7 падала в базову **на кожному прогоні від 03.09**, і виправлення 30.08 «казати на малюнку, що дані симульовані» не спрацювало ЖОДНОГО разу | **ЄДИНИЙ ЖИВИЙ ДЕФЕКТ КОДУ З ЧОТИРНАДЦЯТИ; решта тринадцять — тести, що дивились не туди.** Дві реалізації одного малювання: `reporting.plot_equity_curve` — та, яку **30.08 виправили**, бо прогін написав `equity_curve.png` із випадкових чисел, завершився «Pipeline completed successfully» і лишив людину дивитись на картинку без жодної ознаки, що це не результат; і `ReportGenerator.plot_equity_curve` — класовий метод **без параметра `simulated` і без позначки**. **03.09 оркестратор почав викликати КЛАСОВИЙ із `simulated=`** (комміт `abb770c2`, «етап 7 запрацював уперше, і кожен крок там був хибний по-своєму»). Виклик кидав `TypeError`, широкий `except (ValueError, TypeError, ...)` навколо всієї розгорнутої оцінки його ковтав, і кожен прогін виходив зі статусом `basic_evaluation_not_sent`. **СЛІД НА ДИСКУ, а не лише міркування:** `reports/evaluation/equity_curve.png` — 42 675 байт від **29.08 20:37**, тоді як батч зібрано **02.09**. Файл, куди читач подивиться по останній прогін, не переписувався; поруч `reports/charts/equity_curve.png` (86 512 байт, 30.08) — від тієї, іншої, справної реалізації. **ЧОМУ ЦЕ ГІРШЕ ЗА ЗВИЧАЙНИЙ ЗБІЙ:** усі інші дефекти цього конвеєра віддавали порожнечу; цей віддає щось, схоже на результат. **ВИПРАВЛЕНО ЯК ДУБЛЮВАННЯ, а не як сигнатура:** реалізація тепер одна, класовий метод делегує в неї, а `out_dir` доданий, щоб файл лишився рівно там, куди клас писав завжди. **Перевірено на справжньому об'єкті:** `simulated=False` дає 39 070 байт, `simulated=True` — 72 649, тобто водяний знак реально малюється, і обидва в `reports_dir`. **ЩО ЦЕ КАЖЕ ПРО ТЕСТОВІ ДУБЛЕРИ:** дублер у тесті повторював СТАРУ сигнатуру, і саме тому відтворював збій замість того, щоб його викрити. Це записано в самому дублері. |
| 287 | закрито | дефект | 06.09, закриваючи рекомендацію B із `CRITIQUE.md` | **[метод]** прилад порівнює сиру нетто-Шарпу з планкою, побудованою на константі `SHARPE_SE = 0.193`, — а нуль не там, де вважалось: він не в нулі, він різний на різних горизонтах і різний для різних колонок | **ЗРОБЛЕНО 06.09 — прилад відкалібровано, і три висновки, з яких два розвернули мої власні.** **(1) Р49, підстановка в РЕАЛЬНУ панель:** прилад бачить закладений IC 0.10 із запасом 2.5× над планкою і сліпий нижче 0.05. Оскільки ціль власника (20% річних = Шарп 2.0, Р48) — це саме IC 0.10, **вердикти «нічого не вижило» виявились твердженнями про дані, а не про прилад**: серед 235 колонок немає ефекту ПОТРІБНОГО РОЗМІРУ. Я йшов перевіряти протилежну гіпотезу. **(2) Р50, нуль виміряно двома способами:** обертання позиції в часі (зберігає персистентність) дає розкид 0.394 на h1 і 0.113 на h120 — константа стоїть там, де правда змінюється в 3.5 раза, і хибить у найгіршому напрямку на обох кінцях: занизько на h1, де тертя й так усе з'їдає, зависоко на h120, де за Р27 живе єдина економічно можлива форма. **(3) Р51, найкращий результат проєкту проти ВЛАСНОГО оберненого нуля:** `VOLATILITY_50_1d` h60 має real 0.586 при оберненому середньому **0.559** — тобто вся перевага зберігається, коли позиції зсунути на випадкові дати. Внесок вибору моменту: +0.027, **z=+0.41**. Це статичний нахил (low-vol), а не передбачення. **Вердикт Р28 стоїть — і тепер із правильної причини.** **ЩО ЛИШАЄТЬСЯ ВІДКРИТИМ, і це проводка, не рішення:** обертання зроблено окремим скриптом і **не є частиною приладу** — `net_test_every_survivor.py` досі друкує сиру Шарпу проти константної планки, тобто не відрізняє перевагу від статичної експозиції. Потрібно: (а) рахувати обернений нуль на кожну колонку в самому прогоні й друкувати z поруч із нетто; (б) виміряти розкид на всіх 235 колонках, перш ніж чіпати `SHARPE_SE` — **замінювати константу за чотирма колонками я відмовився свідомо**, це рівно той гріх, який проєкт виправляв тиждень. **ДВІ МОЇ ПОМИЛКИ В ЦІЙ ЖЕ РОБОТІ, обидві зловлені читанням, а не міркуванням:** назвав B «найсильнішою нездійсненою рекомендацією», не прочитавши Р30/Р32, де прогін уже був; і вписав найкраще нетто прогону як **+0.283** замість **+0.586** — з голови, без звірки, — причому помилка підпирала вигідний мені висновок «вердикт стоїть». Друга спіймалась на ROADMAP §29, де число стоїть правильно. **Плюс артефакт власного дизайну, зловлений до публікації:** рядок ρ=0 у сітці підстановки давав розкид 0.018–0.162, і я мало не оголосив усі пороги проєкту завищеними втричі-вдесятеро; насправді така колонка перемальовується щорядка, тож усереднює тисячі витягів за один прогін. Зловлено питанням «що мій дизайн РУЙНУЄ», а не перевіркою числа. **Закріплено:** `tests/contracts/test_the_net_test_recovers_a_planted_edge.py`, 7 тестів. **ПРОДОВЖЕННЯ 06.09 — частину (б) ЗАКРИТО, і прогін дав більше, ніж просив.** Обертання прогнано на **всіх 234 колонках × 6 утриманнях = 1 404 спроби**, поріг оголошено до прогону (Бонферроні z≥4.13). **Розкид виміряно по горизонтах:** 0.293 / 0.125 / 0.077 / 0.061 / 0.056 / 0.044 проти заявлених 0.193 — медіана 0.096, тобто константа завищена вдвічі в середньому й хибна в 6.7 раза між кінцями. **Замінювати `SHARPE_SE` все одно не став:** тепер підстава є, але заміна порогу, під яким лежить уся доказова база, — це окремий комміт із перевіркою всіх цитат, а не побічний ефект діагностики. **ГОЛОВНЕ, ЩО ЗНАЙШЛОСЬ (Р52):** 138 спроб із 1 404 беруть планку — уперше за проєкт, — і **всі на утриманнях 1, 5 і 20 (108/24/7), нуль на 40, 60 і 120**. Інформація обривається рівно там, де тертя перестає кусати. Знайдений ефект — короткостроковий розворот у семи костюмах (`peer_*`, `market_return`, `state_LAG_*`); `market_return` рахується leave-one-out, тож ранжування за ним є ранжуванням за мінус власна дохідність. **Це закриває рекомендацію B по-справжньому:** прилад знайшов ВІДОМИЙ ефект сам, на реальних даних, при z до 10.7 — а не лише впізнав підставлений. **ТРИ ФАЛЬСИФІКАТОРИ ДО ОГОЛОШЕННЯ ЗНАХІДКОЮ:** затримка в один бар (`peer_return`/`market_return` одночасні leave-one-out, тобто припущення нульової затримки — те саме, що вбило заголовок Р32) — **вистояв**, z 10.07 → 10.74; чи міряє z час, а не збіг із дешевими рядками — рахунок за тертя під обертанням змінюється на **≤1.1%**, тобто 4% розриву; гроші окремо від інформації — на 60 обертаннях **жодна з семи не має додатного REAL**. **Розклад на утриманні 20, якого проєкт не мав ніколи: тяга витрат −0.39, інформація +0.23, нетто −0.16.** Інформація покриває 59% витрат. **ЩО ЦЕ ПОТЯГЛО ЗА СОБОЮ (Р53):** розібрано модель витрат, якої ніхто не розбирав — **63% обороту це комісія за акцію**, а не ринок (20.21 bp на іменах дешевших за $20, де комісія 80%; 4.70 bp на дорожчих за $200). Прибирання комісії — це інший БРОКЕР, не параметр, і підстава з'явилась із виміру ДО прогону. Результат: `market_return_1d` h5 дає **REAL +0.210 при z=8.41** — перший додатний нетто в історії проєкту, що ще й бере планку на накопичених 1 471 спробах. **І він мертвий:** «купити все» в тій самій моделі витрат дає **+0.877**, тобто кандидат гірший **вчетверо** (пастка Р28 дослівно), а запас — **один базисний пункт**: при 2 bp виконання результат стає −0.149, і це саме той брокер, чия оплата за потік ордерів робить виконання гіршим. **Важіль реальний — 17.5% річних для будь-якої майбутньої стратегії; стратегії за ним немає.** **ЛИШАЄТЬСЯ ВІДКРИТИМ рівно частина (а):** обертання досі живе в окремому скрипті, а `net_test_every_survivor.py` друкує сиру Шарпу проти константної планки, тобто не відрізняє перевагу від статичного нахилу і не рахує z проти власного нуля колонки. Це проводка на день. **Скрипти:** `what_edge_would_the_net_test_have_seen.py --mode shuffle`, `what_the_costs_are_made_of.py`. **ЗАКРИТО 06.09 — частина (а) зроблена, запис закривається повністю.** Обертання **переїхало в сам прилад** (`net_test_every_survivor.py`), а діагностика тепер бере його звідти (`NET._rotation_index`, `NET._mean_by_date`) — напрямок залежності був неправильний: нуль, який вирішує, як читається вердикт, належить тому, хто вердикт видає, і дві копії одного визначення вже раз дали дві печатки (Р45). **Кожна колонка отримує другий вердикт**, і підсумок розділяє те, що досі було одним числом: знають про КОЛИ — **3**; з них додатні нетто — **3**; **додатні нетто, але НЕ вище власного нуля — 59, тобто нахили, а не переваги**. Останнього рядка прилад не міг надрукувати ніколи, і саме в нього сім днів потрапляв `VOLATILITY_50_1d`. **ТА САМА ПЕРЕВІРКА, ЩО ВБИЛА КАНДИДАТА, ТЕПЕР У ПРИЛАДІ:** він сам порівнює z-виживших із константним суперником **на їхньому ж горизонті** (`vs_opponent`) — усі три програють на 0.54–1.00 Шарпа, і прогін друкує це словами. Досі це порівняння робив я руками після прогону, а перевірка, яку роблять руками, — це перевірка, яку одного разу пропустять (Р28, Р53). **ПЕРЕВІРЕНО, ЩО ПРИЛАД НЕ ЗМІНИВ ЖОДНОГО ЧИСЛА:** 235 колонок × 7 величин проти закоміченого CSV, максимальна різниця **3.6e-15** — машинний нуль; Р22 і Р28 стоять на тих самих числах. Заодно переписано вузьке місце з `groupby` на `bincount`: **114 с на колонку → 15 с**. **МЕЖУ ВЛАСНОЇ КОНСТРУКЦІЇ НАЗВАНО, А НЕ СХОВАНО:** нуль будується лише на найкращому за НЕТТО горизонті, тож родина розворотів (Р52), чиї гроші на 120 днях, а інформація на 1–5, стандартним проходом не побачилася б — для цього є `--rotate-all-holds` (вшестеро дорожче, тому не за замовчуванням), а z пишеться в парі з горизонтом, з якого прийшов (`z_hold`, `net_at_z_hold`). І дванадцять обертань — це **сито, а не цитоване z**: коротким списком займається окремий скрипт із 40–60 обертаннями. **Закріплено трьома контрактами** (набір 472 → 475): обертання не перетинає межу імені; **статичний нахил читається як нахил** (колонка, стала всередині імені, після обертання дає ту саму книгу до 1e-9); швидкий `bincount` збігається з `groupby`, який замінив. |
...
| 138 | закрито | дефект | спостереження v26 | **[конвеєр]** батч пишуть два компоненти; #103 прибрав вибух пам'яті, але не подвійний запис | У прогоні v26 `features.parquet` записано двічі: `pipeline_runner` о 22:32:46 (одразу після «Local pipeline complete», 6405.2 с) і `colab_manager` о 23:15:57, після того як `FeatureLeakageGuard` пройшов 110 тікерів. Обидва рази однаково — 1 242 693 × 2 284, 969 MiB, 11 рядкових груп — тож **цього разу шкоди немає**, а сам повторний запис коштував дві секунди, не сорок три хвилини (проміжок займав сторож витоків). Небезпека структурна: другий писар отримує кадр із власного шляху, і якщо той колись виявиться відфільтрованим чи неповним, він **мовчки перепише добрий батч**, бо жодна перевірка не порівнює те, що вже лежить на диску, з тим, що пишеться. #103 закрив читання-назад і зчеплення батчу з собою; питання «хто власник артефакту» лишилось відкритим. Виправляти після макро: або писар один, або другий порівнює ідентичність і пропускає **ЗАКРИТО 04.09 — мовчання прибрано, власника артефакту не призначав.** Стан підтверджено: `write_union` кличуть з `colab_manager` і `feature_processor`, плюс `colab_manager` пише шлях напряму — три шляхи запису. Писар боронив лише від ПОРОЖНЬОГО запису; з тим, що вже лежить на диску, не порівнював нічого, тож відфільтрований кадр мовчки замінив би добрий батч. **Виправлено:** перед перезаписом читаються МЕТАДАНІ наявного файлу (нуль вартості на 969 МіБ) і перезапис, що ЗМЕНШУЄ рядки чи колонки, доповідається на ERROR із обома формами й дельтою. **Не забороняється:** менший запис буває легітимним (`--tickers AAPL`, прогін одного кадру), а перевірка, що спрацьовує на звичайному, буде вимкнена. **Питання «хто власник артефакту» лишається за власником** — саме воно, а не мовчання, було відкритою половиною. 6 контрактних тестів. **ПОРАХОВАНО 06.09 — це НЕ рішення смаку, і я неправильно назвав його «рішенням власника».** Власник спитав, чи можна порахувати, як краще. Можна, і рахунок дає однозначну відповідь. **(1) Обидва писарі цілять в ОДИН файл.** `pipeline_runner.output_dir` — це `data/colab/accumulated` плюс ім'я батчу, тобто рівно `main_database/features.parquet`, який читає все подальше; `colab_manager` пише туди ж. **(2) Але роблять вони РІЗНЕ, і це головне.** `feature_processor.save_enriched_data` пише кадр ЦЬОГО прогону. `colab_manager._save_frames_by_timeframe` спершу ЧИТАЄ наявний файл, конкатенує з новим, дедуплікує — і аж тоді пише. Тобто це не два писарі одного й того ж, а **виробник → накопичувач**. Саме тому у прогоні v26 обидва записи дали однаковий результат (1 242 693 × 2 284): накопичувач зливав файл сам із собою. **(3) Звідси ризик, який не є справою смаку.** Виробник пише в НАКОПИЧЕНИЙ файл **лише рядки свого прогону**. Якщо накопичувач після цього впаде — впаде він чи прогін, чи машина, — накопичена історія вже перезаписана і зникла. Сторож, доданий при закритті #138, це **доповідає** (ERROR при звуженні рядків або колонок), але **не запобігає**: він читає метадані ПІСЛЯ того, як рішення писати вже ухвалене. **ОБЧИСЛЕНА ВІДПОВІДЬ: власником артефакту має бути НАКОПИЧУВАЧ, а виробник має писати у шлях, прив'язаний до прогону.** «Один писар» правильне не тому, що двоє негарно, а тому що **один із них пише не туди**. Ціна поточного стану виміряна ще при закритті: зайвий повний перезапис 969 МіБ і повне зчитування назад, дві секунди — але дві секунди тут не аргумент, аргумент — вікно, у якому накопичена історія існує лише в одному екземплярі й уже перезаписаному. **Що лишається за власником, і тільки це:** чи міняти шлях запису зараз (зміна торкається розкладки каталогів і всього, що читає батч за іменем) чи після наступного великого прогону. Сам ВИБІР порахований. |
...
| 101 | закрито | дефект | сканер #100 | **[торгівля]** чотири ліміти ризику оголошені в конфігу і не існують у коді | `VirtualPortfolio.__init__` читає з `strategy.risk_management` чотири значення — `max_position_size` (0.1), `max_total_risk` (0.3), `stop_loss_pct` (0.05), `take_profit_pct` (0.1) — і **жодне з них не читає далі жоден рядок**, ні в цьому класі, ні будь-де в `src/`. Перевірено, що це не непрямий доступ: `getattr` у файлі немає. Перевірено й те, чи ліміти не забезпечені іншим механізмом: `buy_stock` відхиляє ордер **лише** за нестачею коштів («Insufficient funds including transaction costs»), а обмеження розміру позиції, сукупного ризику, стоп-лоса й тейк-профіту в класі немає взагалі. Тобто конфіг описує систему керування ризиком, якої нема, і виглядає це як налаштована. **Не виправляю сам**: це не проводка, а вибір політики ризику — числа мають бути рішенням власника, а торгівля зараз стоїть за пайплайном. Коли дійде черга, це також не однорядкове: `max_total_risk` вимагає оцінки ризику **портфеля**, а не суми позицій, і це вже пораховано в #93 і в діверсифікованому VaR. **СТАН ЗАПИСАНО 04.09:** не «невідомо» — #274 перевірив окремо й підтвердив, що ліміти ЖИВІ (присвоєння в рядках 39-42, читачів нуль). Робота відкрита, але числа — рішення власника, не проводка. **ЗАКРИТО 06.09, і моя підстава не робити була ХИБНА.** Я двічі писав, що реалізовувати ліміти передчасно, «бо стратегії немає й торгувати нема чим». **Власник виправив премісу: система планує ПАПЕРОВУ торгівлю, віртуальних грошей необмежено, і саме на них ризик-контроль треба будувати й перевіряти — до того, як це коштуватиме.** Аргумент кращий за мій: `Trader(paper_trading=True)` уже стоїть в оркестраторі, тобто торгівля не «попереду», вона вже тут. **І друга частина моєї підстави теж розсипалась: числа вже є рішенням власника** — 0.10 на позицію, 0.30 сукупно, 0.03 денний збиток, 0.15 просадка записані в конфігу. Дефект був не у відсутності політики, а в тому, що **код її ігнорує**. Тобто це проводка, а не рішення, і я класифікував її неправильно. **ЗРОБЛЕНО:** `_refuse_if_over_limit` у `VirtualPortfolio` — ліміт на одну позицію й на сукупну інвестованість, обидва з ЧИСЕЛ КОНФІГА, перевірка стоїть у `buy_stock` після перевірки коштів (щоб жодна наявна відмова не змінила формулювання, а нові лише додались). **ВИПРАВЛЕНО РОЗБІЖНІСТЬ ІМЕН (#288):** клас просив `max_position_size`, конфіг оголошує `max_position_size_pct` — тепер читається оголошене ім'я, старе лишається як запасне, щоб наявне розгортання не втратило налаштування мовчки. `max_total_risk_pct: 0.30` **оголошено в конфігу — це не нове рішення, а те саме число, що вже було захардкодженим default'ом класу**; тепер воно видиме. **П'ЯТЬ КОНТРАКТІВ, і третій із них головний:** ліміт мусить в'язати на ЧИСЛІ З КОНФІГА, а не на константі — той самий ордер відхиляється при 10% і проходить при 20%, і між прогонами не змінюється нічого, крім налаштування. Плюс: позиції **накопичуються** до одного ліміту (три шматки по 4% не дають обійти 10%), сукупна інвестованість в'яже **через різні імена** (вісім позицій по 5% не проходять), відмова **не рухає ні позиції, ні гроші**. **ПЕРЕВІРЕНО, ЩО ТЕСТИ ЛОВЛЯТЬ СТАРИЙ СТАН:** зі знятою перевіркою **4 з 5 падають**; тест, який проходить і до виправлення, нічого не доводить |
